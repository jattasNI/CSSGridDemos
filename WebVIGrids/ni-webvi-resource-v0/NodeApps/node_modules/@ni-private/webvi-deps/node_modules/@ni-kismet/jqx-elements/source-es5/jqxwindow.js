...
*/
JQX('jqx-window', function (_JQX$ContentElement) {
    babelHelpers.inherits(Window, _JQX$ContentElement);

    function Window() {
        babelHelpers.classCallCheck(this, Window);
        return babelHelpers.possibleConstructorReturn(this, (Window.__proto__ || Object.getPrototypeOf(Window)).apply(this, arguments));
    }
    ...
    babelHelpers.createClass(Window, [{
        ...
        value: function propertyChangedHandler(propertyName, oldValue, newValue) {
            ...
            switch (propertyName) {
                ...
                case 'windowParent':
                    ...
                    break;
                default:
                    babelHelpers.get(Window.prototype.__proto__ || Object.getPrototypeOf(Window.prototype), 'propertyChangedHandler', this).call(this, propertyName, oldValue, newValue);
                    break;
            }
        }
        ...
        */
        value: function ready() {
            babelHelpers.get(Window.prototype.__proto__ || Object.getPrototypeOf(Window.prototype), 'ready', this).call(this);
        ...
        */
        ...
        key: 'collapse',
        value: function collapse(propertyChange) {
            var that = this;
        ...
        key: 'close',
        value: function close(propertyChange) {
            var that = this;
            ...
            if (that.isCompleted) {
                var isOpeningEventPrevented = that.$.fireEvent('closing').defaultPrevented;
        ...
        value: function attached() {
            babelHelpers.get(Window.prototype.__proto__ || Object.getPrototypeOf(Window.prototype), 'attached', this).call(this);
            ...
            if (that._windowParent.parent !== document.body && that._windowParent.parent.querySelectorAll('jqx-window, jqx-dialog-window, jqx-progress-window, jqx-wait-window, ' + 'jqx-alert-window, jqx-prompt-window, jqx-multiline-prompt-window')) {
        ...
        value: function bringToFront() {
            ...
            var parentWindow = that.parentElement.closest('.jqx-window');
            ...
            var windows = document.body.getElementsByClassName('jqx-window');
        ...
        value: function detached() {
            babelHelpers.get(Window.prototype.__proto__ || Object.getPrototypeOf(Window.prototype), 'detached', this).call(this);

            var that = this;

            if (!that._windowParent.parent.querySelectorAll('jqx-window, jqx-dialog-window, jqx-progress-window, jqx-wait-window, ' + 'jqx-alert-window, jqx-prompt-window, jqx-multiline-prompt-window')) {
        ...
        key: 'expand',
        value: function expand(propertyChange) {
            var that = this;
        ...
        key: 'maximize',
        value: function maximize(propertyChange) {
            var that = this;
            ...
            function transitionHandler() {
                ...
                var scrollHeight = that._windowParent.scrollElement.scrollHeight,
        ...
        key: 'minimize',
        value: function minimize(propertyChange) {
            var that = this;
            ...
            var minimizedWindows = that._getAllMinimizedWindows(that._windowParent.element);

            if (minimizedWindows.length > 0) {
                var lastWindow = minimizedWindows[minimizedWindows.length - 1];

                that.style.left = (lastWindow.offsetLeft + lastWindow.offsetWidth + that._dragDetails.minWidth + 10 > that._windowParent.element.clientWidth ? lastWindow.offsetLeft : lastWindow.offsetLeft + lastWindow.offsetWidth + 5) + 'px';
                that.style.top = lastWindow.offsetTop + 'px';
            } else {
                that.style.left = '5px';

                //document.documentElement.scrollTop is always 0 in MS EDGE but not in IE11!
                that.style.top = that._windowParent.element.clientHeight + that._windowParent.scrollElement.scrollTop - that.$.headerSection.offsetHeight - 5 + 'px';
            }
        ...
        */
        ...
        key: 'restore',
        value: function restore(propertyChange) {
            var that = this;
        ...
        key: 'open',
        value: function open(propertyChange) {
            var that = this;
        ...
        */
        ...
        value: function _applyLayoutTemplate(selector, template) {
            var that = this;
        ...
        */
        ...
        key: '_cancelDragging',
        value: function _cancelDragging(event) {
            var that = this;

            if (that._dragDetails && that._dragDetails.started && that._dragDetails.type === 'drag') {
                if (!event) {
                    var windowRect = that.getBoundingClientRect();
        ...
        */
        ...
        key: '_documentFocusInEventHandler',
        value: function _documentFocusInEventHandler(event) {
            var that = this;
        ...
        value: function _downHandler(event) {
            var that = this;
            var target = JQX.Utilities.Core.isMobile ? document.elementFromPoint(event.pageX - window.pageXOffset, event.pageY - window.pageYOffset) : event.originalEvent.target;
            ...
            var isInsideLayout = that.closest('jqx-docking-layout');
        ...
        value: function _documentMoveHandler(event) {
            ...
            if (!that._dragDetails || !that._dragDetails.started || that.minimized || !that._mouseManipulation || Math.abs(event.pageX - that._dragDetails.x) <= 5 && Math.abs(event.pageY - that._dragDetails.y) <= 5) {
                return;
            }
            ...
            if (that._dragDetails.type === 'drag' && !that.pinned) {
                ...
                //document.documentElement.scrollTop is always 0 in MS EDGE but not in IE11!
                that._dragDetails.x = Math.max(that._windowParent.offsetLeft + that._dragDetails.offsetX - that._windowParent.scrollElement.scrollLeft, Math.min(that._windowParent.offsetLeft + that._windowParent.element.offsetWidth - (that._windowParent.element.offsetWidth - that.offsetLeft) + that._dragDetails.offsetX, event.pageX));

                that._dragDetails.y = Math.max(that._windowParent.offsetTop + that._dragDetails.offsetY - that._windowParent.scrollElement.scrollTop, Math.min(that._windowParent.offsetTop + that._windowParent.element.offsetHeight - (that._windowParent.element.offsetHeight - that.offsetTop) + that._dragDetails.offsetY, event.pageY));
                ...
                //document.documentElement.scrollTop is always 0 in MS EDGE but not in IE11!
                that._dragDetails.x = Math.max(that._windowParent.offsetLeft + that._dragDetails.left, Math.min(that._windowParent.offsetLeft + that._dragDetails.left + (that._dragDetails.side.toLowerCase().indexOf('left') > -1 ? 0 : that._dragDetails.newWidth), event.pageX));

                that._dragDetails.y = Math.max(that._windowParent.offsetTop + that._dragDetails.top, Math.min(that._windowParent.offsetTop + that._dragDetails.top + (that._dragDetails.side.toLowerCase().indexOf('top') > -1 ? 0 : that._dragDetails.newHeight), event.pageY));
            }
        }
        ...
        */
        ...
        value: function _documentUpHandler(event) {
            ...
            var closestWindow = target.closest('.jqx-window');
        ...
        key: '_drag',
        value: function _drag(event, direction) {
            var that = this;
            ...
            var totalParentHeight = Math.max(that._windowParent.element.clientHeight, that._windowParent.scrollElement.scrollHeight),
        ...
        */
        ...
        key: '_getAllMinimizedWindows',
        value: function _getAllMinimizedWindows(container) {
            var that = this;
            ...
            var windowElements = container.getElementsByTagName('jqx-window');
        ...
        */
        ...
        key: '_headerDblCickHandler',
        value: function _headerDblCickHandler(event) {
            var that = this,
                target = event.originalEvent.target;
        ...
        */
        ...
        key: '_isWindowButton',
        value: function _isWindowButton(target) {
            var that = this;
            ...
            var customButtonRegex = /jqx-(\w+[-]*\w*)-button/ig;
        ...
        value: function _keyDownHandler(event) {
            ...
            var distance = event.key === 'ArrowDown' || event.key === 'ArrowRight' ? 10 : -10;
            ...
            switch (event.key) {
                ...
                case 'ArrowRight':
                    {
                        ...
                        var direction = event.key.indexOf('Right') > -1 || event.key.indexOf('Left') > -1;
                ...
                case ' ':
                    {
                        var target = event.target.closest && event.target.closest('jqx-button');
        ...
        value: function _mouseWheelHandler(event) {
            ...
            if (event.deltaY > 0 && (that._dragDetails.windowY + that.offsetHeight + event.deltaY >= that._windowParent.scrollElement.scrollHeight || that._windowParent.scrollElement.scrollTop + that._windowParent.element.clientHeight === that._windowParent.scrollElement.scrollHeight)) {
                return;
            }
        ...
        value: function _moveHandler(event) {
            ...
            //Set resizing cursor
            if (that.resizeMode !== 'none') {
                var windowCoordinates = that.getBoundingClientRect(),
        ...
        value: function _handleActiveState() {
            var that = this;

            //Remove the active state of all windows on page
            var allWindowsOnPage = document.getElementsByClassName('jqx-window');

            for (var w = 0; w < allWindowsOnPage.length; w++) {
        ...
        key: '_handleWindowResizeDummy',
        value: function _handleWindowResizeDummy() {
            var that = this;
        ...
        */
        ...
        key: '_resize',
        value: function _resize(side, event) {
            var that = this;
            var size = void 0;
            ...
            var width = that._snapDummy && that.$.hasClass('jqx-window-snapped-' + that._snapDummy._position) ? 'snapWindowWidth' : 'width';

            //document.documentElement.scrollTop is always 0 in MS EDGE but not in IE11!
            switch (side) {
                case 'left':
                    {
                        size = (typeof event === 'undefined' ? 'undefined' : babelHelpers.typeof(event)) === 'object' ? event.pageX - that._dragDetails.x : event;
                ...
                case 'right':
                    {
                        var borderWidth = that._windowParent.element.clientWidth !== that._windowParent.element.offsetWidth ? that._windowParent.borderWidth : 0,
                            _width = that._snapDummy && that.$.hasClass('jqx-window-snapped-' + that._snapDummy._position) ? 'snapWindowWidth' : 'width';

                        size = (typeof event === 'undefined' ? 'undefined' : babelHelpers.typeof(event)) === 'object' ? event.pageX - that._dragDetails.x : event;

                        if (size > 0 && that._dragDetails.maxWidth) {
                            size = Math.min(that._dragDetails.maxWidth - that._dragDetails[_width], size);
                        }

                        that._dragDetails[_width] = Math.min(that._windowParent.element.clientWidth + that._windowParent.scrollElement.scrollLeft - that._dragDetails.left - borderWidth, Math.max(0, that._dragDetails[_width] + size));

                        if ((typeof event === 'undefined' ? 'undefined' : babelHelpers.typeof(event)) !== 'object') {
                            that._dragDetails[_width] = Math.max(that._dragDetails[_width], that._dragDetails.minWidth);
                        }

                        that._dragDetails.left = that.offsetLeft;
                        that._dragDetails.newWidth = Math.max(that._dragDetails.minWidth, that._dragDetails[_width]);
                        break;
                    }
                case 'top':
                    if (that._snapDummy && that.$.hasClass('jqx-window-snapped-' + that._snapDummy._position)) {
                    ...
                    size = (typeof event === 'undefined' ? 'undefined' : babelHelpers.typeof(event)) === 'object' ? event.pageY - that._dragDetails.y : event;
                    ...
                    that._dragDetails.snapWindowHeight = that._dragDetails.height = Math.min(that._dragDetails.height + that._dragDetails.top - that._windowParent.scrollElement.scrollTop, that._dragDetails.height - size);
                ...
                case 'bottom':
                    ...
                    that._dragDetails.snapWindowHeight = that._dragDetails.height = Math.min(that._windowParent.element.clientHeight + that._windowParent.scrollElement.scrollTop - that._dragDetails.top - that._windowParent.borderWidth, Math.max(0, that._dragDetails.height + size));
            ...
            var resizeTarget = that.liveResize ? that : that._resizeDummy;
        ...
        key: '_restoreFromMinimization',
        value: function _restoreFromMinimization() {
            var that = this;
        ...
        key: '_refreshMinimizedWindowsPosition',
        value: function _refreshMinimizedWindowsPosition() {
            var that = this;

            var minimizedWindows = that._getAllMinimizedWindows();
            ...
            var lastWindow = void 0;

            //Using style.left instead of offsetLeft to ignore the transiton of the animation if enabled
            for (var i = 1; i < minimizedWindows.length; i++) {
                if (parseFloat(minimizedWindows[i - 1].style.left) + minimizedWindows[i - 1].offsetWidth + minimizedWindows[i].offsetWidth + 10 < that._windowParent.element.clientWidth) {
        ...
        value: function _setHeaderButtons() {
            var that = this,
                ...
                isAlreadyCreated = function isAlreadyCreated(name) {
                var button = that.$.buttonsContainer.getElementsByClassName('jqx-' + (name + '').split(/(?=[A-Z])/).join('-').toLowerCase() + '-button')[0];
            ...
            for (var b = 0; b < currentButtons.length; b++) {
            ...
            if (buttons.length > 0) {
                ...
                for (var i = 0; i < buttons.length; i++) {
                    var button = isAlreadyCreated(buttons[i]);
        ...
        value: function _setDragDetails(type, event) {
            ...
            if (that._dragDetails.minWidth === undefined || that._dragDetails.minHeight === undefined || that._dragDetails.maxWidth === undefined || that._dragDetails.maxHeight === undefined) {
            ...
            var elementStyle = that.getBoundingClientRect();
        ...
        key: '_setElementParent',
        value: function _setElementParent(id) {
            var that = this;

            that._windowParent.element = undefined;

            if (that._windowParent.parent && !that._windowParent.parent.querySelectorAll('jqx-window, jqx-dialog-window, jqx-progress-window, jqx-wait-window, ' + 'jqx-alert-window, jqx-prompt-window, jqx-multiline-prompt-window')) {
                that._windowParent.parent.style.position = '';
            }
            ...
            if (that._windowParent.element === document.body) {
                ...
                var computedStyle = getComputedStyle(that.parentElement),
        ...
        value: function _setModal() {
            ...
            if (that.opened && !that._modal.parentElement) {
                var zIndex = parseInt(getComputedStyle(that).getPropertyValue('z-index'));
...
* jqxDialogWindow
*/
JQX('jqx-dialog-window', function (_JQX$Window) {
    babelHelpers.inherits(DialogWindow, _JQX$Window);

    function DialogWindow() {
        babelHelpers.classCallCheck(this, DialogWindow);
        return babelHelpers.possibleConstructorReturn(this, (DialogWindow.__proto__ || Object.getPrototypeOf(DialogWindow)).apply(this, arguments));
    }
...
JQX('jqx-alert-window', function (_JQX$DialogWindow) {
    babelHelpers.inherits(AlertWindow, _JQX$DialogWindow);

    function AlertWindow() {
        babelHelpers.classCallCheck(this, AlertWindow);
        return babelHelpers.possibleConstructorReturn(this, (AlertWindow.__proto__ || Object.getPrototypeOf(AlertWindow)).apply(this, arguments));
    }

    babelHelpers.createClass(AlertWindow, null, [{
        key: 'properties',
...
JQX('jqx-prompt-window', function (_JQX$DialogWindow2) {
    babelHelpers.inherits(PromptWindow, _JQX$DialogWindow2);

    function PromptWindow() {
        babelHelpers.classCallCheck(this, PromptWindow);
        return babelHelpers.possibleConstructorReturn(this, (PromptWindow.__proto__ || Object.getPrototypeOf(PromptWindow)).apply(this, arguments));
    }

    babelHelpers.createClass(PromptWindow, [{
        key: 'template',
        ...
        value: function _keyDownHandler(event) {
            ...
            switch (event.key) {
                ...
                case 'ArrowRight':
                    ...
                    break;
                case 'Escape':
                    if (that.headerButtons.indexOf('close') > -1 && document.activeElement === that) that.close();
                    break;
                case 'Tab':
                    if (that.opened && that.modal) {
                        that._changingFocus = true;
...
JQX('jqx-multiline-prompt-window', function (_JQX$PromptWindow) {
    babelHelpers.inherits(MultiLinePromptWindow, _JQX$PromptWindow);

    function MultiLinePromptWindow() {
        babelHelpers.classCallCheck(this, MultiLinePromptWindow);
        return babelHelpers.possibleConstructorReturn(this, (MultiLinePromptWindow.__proto__ || Object.getPrototypeOf(MultiLinePromptWindow)).apply(this, arguments));
    }

    babelHelpers.createClass(MultiLinePromptWindow, [{
        key: 'template',
    ...
    return MultiLinePromptWindow;
}(JQX.PromptWindow));

JQX('jqx-progress-window', function (_JQX$Window2) {
    babelHelpers.inherits(ProgressWindow, _JQX$Window2);

    function ProgressWindow() {
        babelHelpers.classCallCheck(this, ProgressWindow);
        return babelHelpers.possibleConstructorReturn(this, (ProgressWindow.__proto__ || Object.getPrototypeOf(ProgressWindow)).apply(this, arguments));
    }

    babelHelpers.createClass(ProgressWindow, [{
        key: 'template',
        ...
        */
        value: function propertyChangedHandler(propertyName, oldValue, newValue) {
            ...
            switch (propertyName) {
                case 'value':
                    ...
                    break;
                default:
                    babelHelpers.get(ProgressWindow.prototype.__proto__ || Object.getPrototypeOf(ProgressWindow.prototype), 'propertyChangedHandler', this).call(this, propertyName, oldValue, newValue);
                    break;
            }
        }
        ...
        value: function _handleCompletion() {
            var that = this;
...
JQX('jqx-tabs-window', function (_JQX$Window3) {
    babelHelpers.inherits(TabsWindow, _JQX$Window3);

    function TabsWindow() {
        babelHelpers.classCallCheck(this, TabsWindow);
        return babelHelpers.possibleConstructorReturn(this, (TabsWindow.__proto__ || Object.getPrototypeOf(TabsWindow)).apply(this, arguments));
    }

    babelHelpers.createClass(TabsWindow, [{
        key: 'template',
        ...
        */
        ...
        value: function propertyChangedHandler(propertyName, oldValue, newValue) {
            ...
            if (propertyName !== 'collapsed') {
                babelHelpers.get(TabsWindow.prototype.__proto__ || Object.getPrototypeOf(TabsWindow.prototype), 'propertyChangedHandler', this).call(this, propertyName, oldValue, newValue);
            }
        ...
        value: function _applyTheme(theme) {
            ...
            if (theme !== '') {
                var headerButtons = that.$.buttonsContainer.children;
        ...
        key: '_autoHideWindowOpenHandler',
        value: function _autoHideWindowOpenHandler() {
            var that = this;
        ...
        **/
        ...
        key: '_handleLayoutProperty',
        value: function _handleLayoutProperty() {
            var that = this,
        ...
        */
        ...
        value: function _headerDblCickHandler(event) {
            ...
            if (event.ctrlKey || target !== that.$.headerSection && target !== that.$.header || that.$.maximizeButton && that.$.maximizeButton.offsetHeight === 0) {
                return;
            }
        ...
        */
        ...
        value: function _documentUpHandler(event) {
            ...
            babelHelpers.get(TabsWindow.prototype.__proto__ || Object.getPrototypeOf(TabsWindow.prototype), '_documentUpHandler', this).call(this, event);
        ...
        key: '_handleAutoHide',
        value: function _handleAutoHide(index) {
            var that = this;
            ...
            var targetTab = that.$.tabsElement._tabs[index];
        ...
        value: function _moveHandler(event) {
            ...
            if (isInsideLayout) {
                var layoutItems = isInsideLayout.items;
            ...
            //iOS Safari fix for dragging (prevents window scrolling)
            if (JQX.Utilities.Core.isMobile && !!navigator.platform && /iPad|iPhone|iPod/.test(navigator.platform) && that.layout instanceof JQX.DockingLayout && that.layout.hasAttribute('dragged')) {
                event.originalEvent.preventDefault();
            }

            babelHelpers.get(TabsWindow.prototype.__proto__ || Object.getPrototypeOf(TabsWindow.prototype), '_moveHandler', this).call(this, event);
        }
        ...
        key: '_setAutoHideWindowSize',
        value: function _setAutoHideWindowSize(selectedIndex) {
            var that = this;
            ...
            var targetTab = that.$.tabsElement._tabs[selectedIndex],
            ...
            if (that.tabPosition === 'top' || that.tabPosition === 'bottom') {
                ...
                var height = targetTab._autoHideWindowSize ? parseFloat(targetTab._autoHideWindowSize) || '' : '';
                ...
                var width = targetTab._autoHideWindowSize ? parseFloat(targetTab._autoHideWindowSize) || '' : '';
        ...
        value: function _handleAutoHideWindow(createWindow) {
            ...
            var autoHideWindow = document.createElement('jqx-window');
        ...
        key: '_moveContent',
        value: function _moveContent(from, to) {
            if (!from || !to) {
                return;
            }

            var contentNodes = [].slice.call(from.$.content.childNodes);
        ...
        key: '_tabsChangeHandler',
        value: function _tabsChangeHandler(event) {
            var that = this;
            ...
            if (event.detail.oldIndex !== null && that._autoHideWindow.opened) {
                var previousTabItem = that.$.tabsElement._tabs[event.detail.oldIndex];

                previousTabItem._autoHideWindowSize = that._autoHideWindow.style[that.$.tabsElement.tabPosition === 'right' || that.$.tabsElement.tabPosition === 'left' ? 'width' : 'height'];
            }
        ...
        key: '_tabsFocusHandler',
        value: function _tabsFocusHandler(event) {
            var that = this;
        ...
        */
        ...
        get: function get() {
            ...
            var closestSplitter = that.closest('jqx-splitter');

            if (!closestSplitter || !closestSplitter.closest('jqx-docking-layout')) {
                return siblings;
            }

            var items = closestSplitter._items;
            ...
            for (var s = 0; s < items.length; s++) {
                ...
                if (item !== that.closest('jqx-splitter-item')) {
                    var siblingItem = item.querySelector('jqx-tabs-window');
...
JQX('jqx-wait-window', function (_JQX$ProgressWindow) {
    babelHelpers.inherits(WaitWindow, _JQX$ProgressWindow);

    function WaitWindow() {
        babelHelpers.classCallCheck(this, WaitWindow);
        return babelHelpers.possibleConstructorReturn(this, (WaitWindow.__proto__ || Object.getPrototypeOf(WaitWindow)).apply(this, arguments));
    }

    babelHelpers.createClass(WaitWindow, [{
        key: 'template',
