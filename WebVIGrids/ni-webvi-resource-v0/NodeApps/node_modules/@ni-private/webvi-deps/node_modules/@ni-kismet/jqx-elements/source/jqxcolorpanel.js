...
JQX('jqx-color-panel', class ColorPanel extends JQX.BaseElement {
    ...
    propertyChangedHandler(propertyName, oldValue, newValue) {
        ...
        switch (propertyName) {
            ...
            case 'paletteColors':
                ...
                break;
            case 'inverted': {
                const hueScaleThumbHeight = that.$.hueScaleThumb.offsetHeight,
    ...
    */
    _containerClickHandler(event) {
        ...
        const isButton = event.target.closest('jqx-button');
    ...
    _moveThumbHandler(event, customColorSelect) {
        ...
        switch (that._activeComponent) {
            ...
            case (that.$.colorPaletteRadial): {
                ...
                const previewContainerBottom = that.querySelector('.preview-container-bottom');
            ...
            case (that.$.brightnessScale): {
                ...
                if (that.displayMode === 'hexagonal') {
                    const customHSV = that.inverted ? { s: that._HSV.s } : { v: that._HSV.v };
            ...
            case (that.$.alphaChannelScale): {
                const thumbCenter = coord.x - that.$.alphaChannelThumb.offsetWidth / 2,
    ...
    */
    _handleCancelButtonClick(fireClickEvent) {
        ...
        if (that.displayMode === 'palette' || that.displayMode === 'radial') {
            const previewContainerTop = that.querySelector('.preview-container-top');
    ...
    */
    _mapDataSourceToPalette() {
        ...
        for (let i = 0; i < that.paletteColors.length; i++) {
            const item = that.paletteColors[i];

            if (typeof item === 'string') {
                customPalette.push(that.paletteColors[i]);
            }
            else if (typeof item === 'object' && ((item.value && typeof item === 'string') || (item[that.valueMember] && typeof item[that.valueMember] === 'string'))) {
                customPalette.push(that.paletteColors[i]);
            }
            else {
                customPalette.push(that.paletteColors[i]);
    ...
    */
    _changeSampleSelection(item) {
        ...
        Array.from(that.getElementsByClassName('selected')).forEach(item => {
    ...
    _generateMaterialGrid() {
        ...
        Object.entries(that._materialColors).forEach(gama => {
            ...
            Object.entries(gama[1]).forEach(color => {
                const colorSample = that._createColorSample(color[1], 'material-color');
                fragment.appendChild(colorSample);
            });
        });
    ...
    _hideControlsByPriority() {
        ...
        function hideComponent(className) {
            ...
            for (let i = 0; i < components.length; i++) {
                components[i].classList.add('jqx-hidden');
            }
        }
    ...
    */
    _getRGBAFromInputs() {
        const that = this;
        let r = that.querySelector('input.r-channel'),
    ...
    _setRGBAToInputs() {
        ...
        Object.entries(that._RGBA).forEach(entry => {
            const input = that.querySelector('input.' + entry[0] + '-channel');
