...
JQX('jqx-numeric-text-box', function (_JQX$BaseElement) {
    ...
    babelHelpers.createClass(NumericTextBox, [{
        ...
        key: '_refreshShape',
        value: function _refreshShape() {
            var that = this;

            var addons = that.$.container.querySelectorAll('.jqx-numeric-text-box-component:not(.jqx-hidden)');
        ...
        value: function val(value, suppressValidation) {
            var that = this,
                isEmptyObject = value !== null && (typeof value === 'undefined' ? 'undefined' : babelHelpers.typeof(value)) === 'object' && Object.keys(value).length === 0;
        ...
        value: function _validate(initialValidation, programmaticValue) {
            ...
            var validNumber = that._numericProcessor.createDescriptor(validationOptions.value, true, true, !initialValidation && programmaticValue === undefined || that.validation === 'strict', initialValidation || programmaticValue !== undefined, validationOptions.enteredComplexNumber);
        ...
        value: function _handleInfinity(initialValidation, programmaticValue, value) {
            ...
            if (newValue === -Infinity && that.min === -Infinity || newValue === Infinity && that.max === Infinity || programmaticValue !== undefined) {
                if (initialValidation) {
                    that.value = newValue;
                    ...
                    case 'wordLength':
                        ...
                        if (that.inputFormat === 'integer') {
                            ...
                            if (that._number !== null) {
                                var _validValue = that._validateRange(new JQX.Utilities.BigNumber(that._number));
        ...
        value: function _updateValue(value) {
            ...
            if (enteredValue !== newValue || enteredValue !== that._cachedInputValue) {
                ...
                if (that._inputFormatChangedFlag || newValueIsNotNumeric && renderedValue !== oldValue || newValueIsNotNumeric === false && that._numericProcessor.compare(value, that._number)) {
        ...
        value: function _openRadix() {
            ...
            var windowHeight = window.devicePixelRatio === 1 ? document.documentElement.clientHeight : window.innerHeight,
        ...
        value: function _documentMousedownHandler(event) {
            ...
            if (that.enableShadowDOM) {
                var shadowRootTarget = event.originalEvent.composedPath()[0],
                    isInsideElement = function () {
                    var host = shadowRootTarget.getRootNode().host;
        ...
        key: '_dropDownItemClickHandler',
        value: function _dropDownItemClickHandler(event) {
        ...
        value: function _inputKeyupHandler(event) {
            ...
            if (event.keyCode === 13) {
                ...
                if (that._keydownInfo && that._keydownInfo.value !== inputValue && !that._keydownInfo.specialKey && !event.altKey && !event.ctrlKey && !event.shiftKey && event.key !== 'Control') {
        ...
        value: function _inputPasteHandler() {
            var that = this;

            requestAnimationFrame(function () {
                return that.$.fireEvent('changing', { 'currentValue': that.$.input.value, 'validValue': that.value, 'radix': that._radixNumber });
            });
        }
        ...
        key: '_setDropDownOptions',
        value: function _setDropDownOptions() {
            var that = this;
        ...
        value: function _incrementOrDecrement(func) {
            var that = this,
                activeElement = that.enableShadowDOM ? that.shadowRoot.activeElement || document.activeElement : document.activeElement;
            var cachedInputValue = that._cachedInputValue;
        ...
        value: function _getBigNumberFrom64BitBinOctHex(number, radix) {
            var result = new JQX.Utilities.BigNumber(0);
            for (var i = number.length - 1; i >= 0; i--) {
                var current = new JQX.Utilities.BigNumber(parseInt(number.charAt(i), radix));
        ...
        key: '_renderValue',
        value: function _renderValue(renderedValue, valueInDropDown) {
            var that = this,
                value = renderedValue,
        ...
        key: '_disableComponents',
        value: function _disableComponents() {
            var that = this;
        ...
        key: 'styleUrls',
        get: function get() {
            return ['jqx.button.css', 'jqx.numerictextbox.css'];
        }
    }]);
