...
*/
JQX('jqx-calendar', function (_JQX$BaseElement) {
    babelHelpers.inherits(Calendar, _JQX$BaseElement);

    function Calendar() {
        babelHelpers.classCallCheck(this, Calendar);
        return babelHelpers.possibleConstructorReturn(this, (Calendar.__proto__ || Object.getPrototypeOf(Calendar)).apply(this, arguments));
    }
    ...
    babelHelpers.createClass(Calendar, [{
        ...
        */
        ...
        value: function propertyChangedHandler(propertyName, oldValue, newValue) {
            babelHelpers.get(Calendar.prototype.__proto__ || Object.getPrototypeOf(Calendar.prototype), 'propertyChangedHandler', this).call(this, propertyName, oldValue, newValue);
            ...
            switch (propertyName) {
                ...
                case 'dateFormatFunction':
                    {
                        var months = that.$.monthsContainer.children;

                        for (var m = 0; m < months.length; m++) {
                            months[m].firstElementChild.innerHTML = newValue ? newValue(months[m]._date) : that._monthsNames[months[m]._date.getMonth()] + ' ' + months[m]._date.toLocaleDateString(that.locale, { year: that.yearFormat });
                        }
                ...
                case 'selectedDates':
                    ...
                    if (that._viewDates) {
                        var lastSelectedDate = that.selectedDates[that.selectedDates.length - 1];

                        !that._isDateInView(lastSelectedDate) ?
                        //true flags indicate eventPrevention and noPropertySynchronization
                        that._handleMonths(lastSelectedDate) : that.selectedDates.map(function (date) {
                            return that._selectDate(date, true, true);
                        });
                    }
                ...
                case 'yearFormat':
                    ...
                    if (that.months > 1 && that.displayMode === 'month') {
                        var _months = that._animationStarted ? that.$.nextMonthsContainer.children : that.$.monthsContainer.children;

                        for (var i = 0; i < _months.length; i++) {
                            _months[i].firstElementChild.innerHTML = that.dateFormatFunction ? that.dateFormatFunction(_months[i]._date) : that._monthsNames[_months[i]._date.getMonth()] + ' ' + _months[i]._date.toLocaleDateString(that.locale, { year: newValue });
                        }
                    }
        ...
        */
        ...
        value: function ready() {
            babelHelpers.get(Calendar.prototype.__proto__ || Object.getPrototypeOf(Calendar.prototype), 'ready', this).call(this);
        ...
        key: '_updateViewSectionsVisibility',
        value: function _updateViewSectionsVisibility() {
            var that = this;
        ...
        value: function _setFocusable() {
            ...
            function setHeaderElement(component, componentType) {
                ...
                if (component && component instanceof componentType) {
                    if (typeof component._setFocusable === 'function' && component.isCompleted) {
                        //If a JQX element is added to the element and its not ready
        ...
        */
        value: function navigate(step) {
            ...
            var navigationDate = that._getNextDate(step);
        ...
        */
        ...
        value: function _animateNavigation(navigationDate, step, completeAnimation) {
            var that = this;

            that._navigationDate = navigationDate;

            function scroll() {
                var endCondition = step > 0 ? that.$.body[that._animationSettings.scrollSize] >= scrollMax : that.$.body[that._animationSettings.scrollSize] === 0,
                ...
                if (step > 0 && that.$.body[that._animationSettings.scrollSize] > scrollMax * that._animationSettings.easeThreshold) {
                    coefficient = Math.max(that.$.body[that._animationSettings.scrollSize] / that._animationSettings.stepEaseSize, Math.abs(coefficient - (scrollMax - that.$.body[that._animationSettings.scrollSize]) * that._animationSettings.step));
                    ...
                    coefficient = Math.max(that.$.body[that._animationSettings.scrollMax] / that._animationSettings.stepEaseSize, Math.abs(coefficient - that.$.body[that._animationSettings.scrollSize] * that._animationSettings.step));
                    that._scrollStep = that._scrollStep * that._animationSettings.step + coefficient;
                }
            ...
            var animationTarget = that.displayMode !== 'month' ? that.$.dateViewContainer : that.$.monthsContainer,
            ...
            if (!completeAnimation) {
                if (step > 0) {
                    ...
                    if (!that._animationStarted || that._animationStarted && that.$.body[that._animationSettings.scrollSize] > that.$.body[that._animationSettings.scrollMax] * that._animationSettings.resetThreshold * 2) {
                        that.$.body[that._animationSettings.scrollSize] = 0;
                    }
                } else {
                    animationTarget.style.order = 3;
                    that.$.nextMonthsContainer.style.order = 1;
                    if (!that._animationStarted || that._animationStarted && that.$.body[that._animationSettings.scrollSize] < that.$.body[that._animationSettings.scrollMax] * that._animationSettings.resetThreshold) {
        ...
        key: '_animateNavigationCompleted',
        value: function _animateNavigationCompleted() {
            var that = this;
            var animationTarget = that.displayMode !== 'month' ? that.$.dateViewContainer : that.$.monthsContainer,
                focusedDate = void 0,
                newCells = void 0,
            ...
            if (that.displayMode !== 'month') {
                var focusedItemIndex = [].slice.call(animationTarget.children).indexOf(that._focusedCell);

                animationTarget.innerHTML = that.$.nextMonthsContainer.innerHTML;

                var focusedItem = animationTarget.children[focusedItemIndex],
                    lastFocusableItem = void 0;
                ...
                var newMonths = animationTarget.children,
                    months = that.$.nextMonthsContainer.children;

                for (var _i = 0; _i < newMonths.length; _i++) {
                    newMonths[_i]._date = months[_i]._date;
                    newMonths[_i].innerHTML = months[_i].innerHTML;
                    newCells = that._getMonthCells(newMonths[_i]);
                    oldCells = that._getMonthCells(months[_i]);

                    for (var _i2 = 0; _i2 < newCells.length; _i2++) {
                        newCells[_i2].value = oldCells[_i2].value;
                        newCells[_i2].restricted = oldCells[_i2].restricted;
                        newCells[_i2].important = oldCells[_i2].important;
                        newCells[_i2].otherMonth = oldCells[_i2].otherMonth;
                    }
                }

                //Synch _selectedCells
                if (that._selectedCells.length > 0) {
                    that._selectedCells = that._selectedCells.map(function (date) {
                        return date = that._getCellByDate(date.value);
        ...
        key: '_applyAnimationSettings',
        value: function _applyAnimationSettings(displayModeChangeOnly) {
            var that = this;
        ...
        */
        ...
        key: '_applySelectionMode',
        value: function _applySelectionMode(initial) {
            var that = this;
            var newDate = [],
            ...
            switch (that.selectionMode) {
                ...
                case 'week':
                    if (that.selectedDates.length >= 1) {
                        var date = that.selectedDates[that.selectedDates.length - 1];
            ...
            var newDates = newDate.length;

            for (var d = 0; d < newDates; d++) {
        ...
        */
        ...
        key: '_clearSelection',
        value: function _clearSelection(eventPrevented) {
            var that = this;

            if (that._selectedCells) {
                that._selectedCells.map(function (cell) {
            ...
            if (that.selectionMode === 'many') {
                ...
                for (var m = 0; m < months.length; m++) {
                    that._getMonthCells(months[m]).map(function (cell) {
        ...
        key: '_bodyBlurStateHandler',
        value: function _bodyBlurStateHandler(event) {
            var that = this;

            that._bodyStateHandler(event);
        }
    }, {
        key: '_bodyFocusStateHandler',
        value: function _bodyFocusStateHandler(event) {
            var that = this;
        ...
        */
        ...
        key: '_bodyStateHandler',
        value: function _bodyStateHandler(event) {
            var that = this;
        ...
        */
        ...
        key: '_containerWheelHandler',
        value: function _containerWheelHandler(event) {
            var that = this;
        ...
        */
        ...
        value: function _downHandler(event) {
            ...
            if (that.hasRippleAnimation) {
                if (target.value instanceof Date || target.parentElement && target.parentElement.classList.contains('jqx-calendar-title') || target.parentElement === that.$.dateElement) {
        ...
        key: '_displayModeHandler',
        value: function _displayModeHandler(target, newDisplayMode) {
            var that = this;
            ...
            if (target instanceof HTMLElement && target.parentElement.classList.contains('jqx-calendar-week') && target.otherMonth) {
                if (that.displayMode === 'decade' && (target.value.getFullYear() < that.min.getFullYear() || target.value.getFullYear() > that.max.getFullYear())) {
                    return;
                } else if (that.displayMode === 'year' && (target.value.getFullYear() < that.min.getFullYear() || target.value.getFullYear() === that.min.getFullYear() && target.value.getMonth() < that.min.getMonth() || target.value.getFullYear() > that.max.getFullYear() || target.value.getFullYear() === that.max.getFullYear() && target.value.getMonth() > that.max.getMonth())) {
                    return;
                }
            }
            ...
            var isDisplayModeCHangeEventPrevented = that.$.fireEvent('displayModeChanging', {
            ...
            if (that.hasAnimation) {
                if (target instanceof JQX.ListItem || target.ownerListBox === that.$.listViewContainer) {
                    if (target.offsetHeight) {
                        that.$.animationPanel.style.transformOrigin = target.offsetLeft + target.offsetWidth / 2 + 'px ' + (target.dataItem.top > 0 ? target.offsetTop - target.offsetHeight / 2 : target.offsetTop + target.offsetHeight / 2) + 'px';
                    } else {
                        that.$.animationPanel.style.transformOrigin = that.$.listViewContainer.offsetLeft + that.$.listViewContainer.offsetWidth / 2 + 'px ' + (that.$.listViewContainer.offsetTop + that.$.listViewContainer.offsetHeight / 2) + 'px';
                    }
                } else {
        ...
        key: '_animateView',
        value: function _animateView(direction, target) {
            var that = this;

            var date = target instanceof Date ? target : new Date(target.value),
                endCondition = false,
                scale = 1,
                opacity = 1,
                animationTarget = void 0,
                backgroundContainer = void 0;

            if (that.displayModeView === 'list') {
                animationTarget = target instanceof JQX.ListItem || target.ownerListBox === that.$.listViewContainer ? that.$.listViewContainer : that.displayMode !== 'month' ? that.$.monthsContainer : that.$.listViewContainer;
            ...
            //else if (!(target instanceof JQX.cell)) {
                    ...
                    target = that._focusedCell ? that._focusedCell : that._selectedCells.length > 0 ? that._selectedCells[that._selectedCells.length - 1] : that._getCellByDate(new Date());
                }
            ...
            if (animationTarget instanceof JQX.ListBox) {
                var listBox = that.$.animationPanel.children[0];
        ...
        value: function _documentUpHandler(event) {
            ...
            if (Math.abs(that._dragStartDetails[dragging.coordinate] - event[dragging.direction]) <= 10) {
                ...
                var speed = Math.abs((event[dragging.direction] - that._dragStartDetails[dragging.coordinate]) / (timeNow - that._dragStartDetails.startTime));

                if (speed > 1) {
                    var steps = parseInt(speed); //Determine the animation iterations
            ...
            var scrollMax = (that.displayMode !== 'month' ? that.$.dateViewContainer[that._animationSettings.scrollMax] : that.$.monthsContainer[that._animationSettings.scrollMax]) + that.$.monthsSeparator[that._animationSettings.size];

            if (that._dragStartDetails.step > 0 && that.$.body[that._animationSettings.scrollSize] > scrollMax * that._animationSettings.easeThreshold || that._dragStartDetails.step < 0 && that.$.body[that._animationSettings.scrollSize] < scrollMax * that._animationSettings.easeThreshold) {
        ...
        key: '_animateMonthRepositioning',
        value: function _animateMonthRepositioning(navigationDate, step) {
            var that = this,
            ...
            function scroll() {
                ...
                if (step > 0) {
                    ...
                    coefficient = Math.max(that.$.body[that._animationSettings.scrollMax] / that._animationSettings.stepEaseSize, Math.abs(coefficient - (scrollMax - that.$.body[that._animationSettings.scrollSize]) * that._animationSettings.step));
                    that._scrollStep = that._scrollStep * that._animationSettings.step + coefficient;
                }
            ...
            var scrollMax = animationTarget[that._animationSettings.scrollMax] + that.$.monthsSeparator[that._animationSettings.size];
        ...
        */
        ...
        key: '_focusCell',
        value: function _focusCell(cell) {
            var that = this;
            ...
            if (that._selectedCells.length > 0) {
                var lastSelectedCell = that._selectedCells[that._selectedCells.length - 1];
                ...
                var monthCells = that._getMonthCells(months[0], that.rightToLeft);

                for (var d = 0; d < monthCells.length; d++) {
                    var _cell = monthCells[d];

                    if (!_cell.otherMonth) {
                        if (_cell.disabled || _cell.restricted) {
                            continue;
                        }

                        that._focusedCell = _cell;
                        break;
                    }
                }
            }
        ...
        */
        ...
        key: '_getDays',
        value: function _getDays(date, datePool) {
            var datesCount = datePool.length;
            var days = [];

            if (datesCount > 0) {
                for (var d = 0; d < datesCount; d++) {
        ...
        */
        ...
        key: '_getCellByDate',
        value: function _getCellByDate(date, monthContainer) {
            var that = this;
            ...
            if (that.displayMode !== 'month') {
                ...
                var items = arguments[1] ? that.$.nextMonthsContainer.children : that.$.dateViewContainer.children;
            ...
            var ownerMonth = void 0,
                monthCells = void 0,
                months = monthContainer.children;

            for (var _i3 = 0; _i3 < months.length; _i3++) {
                if (date.getFullYear() === months[_i3]._date.getFullYear() && date.getMonth() === months[_i3]._date.getMonth()) {
                    ownerMonth = months[_i3];
                    break;
                }
            }
            ...
            for (var _i4 = 0; _i4 < monthCells.length; _i4++) {
                if (monthCells[_i4].otherMonth && !monthCells[_i4].hover) {
                    continue;
                }

                if (monthCells[_i4].value.getMonth() === date.getMonth() && monthCells[_i4].value.getDate() === date.getDate()) {
                    return monthCells[_i4];
                }
            }
        }
        ...
        */
        ...
        key: '_getMonthCells',
        value: function _getMonthCells(month, rightToLeft) {
            var weeks = month.getElementsByClassName('jqx-calendar-week');
            var days = [];

            //Get days instances.
            if (rightToLeft) {
                for (var w = 0; w < weeks.length; w++) {
                    var weekDays = weeks[w].children;

                    for (var d = weekDays.length - 1; d > 0; d--) {
                        days.push(weekDays[d]);
                    }
                }
            } else {
                for (var _w = 0; _w < weeks.length; _w++) {
                    var _weekDays = weeks[_w].children;

                    for (var _d = 1; _d < _weekDays.length; _d++) {
                        days.push(_weekDays[_d]);
                    }
                }
            }
        ...
        */
        ...
        key: '_getWeekNumber',
        value: function _getWeekNumber(date) {
        ...
        key: '_handleDateSelection',
        value: function _handleDateSelection(cell) {
            var that = this;
            ...
            switch (that.selectionMode) {
                ...
                case 'many':
                    {
                        ...
                        var lastSelectedDate = that.selectedDates[that.selectedDates.length - 1];
                        var firstSelectedDate = new Date(that.selectedDates[0]);
                ...
                case 'week':
                    {
                        if (cell.selected && (cell.value.getTime() === that.selectedDates[0].getTime() || cell.value.getTime() === that.selectedDates[that.selectedDates.length - 1].getTime())) {
                            that._clearSelection();
                            that._focusCell(cell);
                            return;
                        }
        ...
        */
        ...
        key: '_handleImportantDateTemplate',
        value: function _handleImportantDateTemplate() {
            var that = this;
            ...
            var content = that._importantDatesTemplate.content;
            var childrenCount = content.childNodes.length,
                regex = /{{\w+}}/g;
        ...
        */
        ...
        key: '_handleLayoutTemplate',
        value: function _handleLayoutTemplate(selector, newTemplate) {
            var that = this,
                template = that._validateTemplate(selector.$.hasClass('jqx-header') ? 'headerTemplate' : selector.$.hasClass('jqx-calendar-footer') ? 'footerTemplate' : 'titleTemplate', newTemplate);
            ...
            var childrenCount = clone.childNodes.length,
                regex = /{{\w+}}/g;
            var bindingString = void 0,
                bindings = [];
            ...
            that._layoutTemplates = that._layoutTemplates.filter(function (template) {
                return template.selector !== selector;
            });
        ...
        */
        ...
        key: '_handleCalendarCellHover',
        value: function _handleCalendarCellHover(event, cell) {
            var that = this;
            ...
            if (that.displayMode === 'month') {
                if (event.type === 'mouseover' && that.selectionMode === 'many' && that.selectedDates.length > 0) {
                    ...
                    var firstSelectedDate = new Date(that.selectedDates[0]),
                        ...
                        targetCell = cell;
                    var nextCoeff = firstSelectedDate.getTime() > targetDate.getTime() ? -1 : 1,
                        hoverCell = function hoverCell() {
        ...
        key: '_handleMonths',
        value: function _handleMonths(date, monthsContainer) {
            var that = this;
            var months = void 0;
            ...
            function populateMonths(monthsContainer) {
                var months = monthsContainer.children;
        ...
        */
        ...
        key: '_headerChangeHandler',
        value: function _headerChangeHandler(event) {
            var that = this;
            ...
            if (event.target === that.$.yearElement) {
                that._viewDates[0].setFullYear(Math.min(Math.max(that.min.getFullYear(), isNaN(parseInt(that.$.yearElement.value)) ? 0 : parseInt(that.$.yearElement.value)), that.max.getFullYear()));
        ...
        */
        ...
        value: function _headerClickHandler(event) {
            ...
            var target = event.target.closest('jqx-repeat-button');
        ...
        key: '_setDisplayMode',
        value: function _setDisplayMode(oldValue, newValue, animationTarget) {
            var that = this;
        ...
        value: function _setDisplayModeContent(newDate, nextDateViewContainer) {
            var that = this;
            var selectedDate = !newDate || newDate.toString() === 'Invalid Date' ? that.selectedDates.length > 0 ? that.selectedDates[that.selectedDates.length - 1] : new Date() : newDate;

            selectedDate.setTime(Math.min(Math.max(selectedDate.getTime(), that.min.getTime()), that.max.getTime()));

            selectedDate.setHours(0, 0, 0, 0);

            var viewDetails = that._prepareView(selectedDate);

            if (!viewDetails) {
                return;
            }

            var dateViewContainer = void 0;
            ...
            var cells = dateViewContainer.children;
            ...
            for (var i = 0; i < cells.length; i++) {
                ...
                if (that.displayMode === 'year') {
                    if (cell.value.getFullYear() < that.min.getFullYear() || cell.value.getFullYear() === that.min.getFullYear() && cell.value.getMonth() < that.min.getMonth() || cell.value.getFullYear() > that.max.getFullYear() || cell.value.getFullYear() === that.max.getFullYear() && cell.value.getMonth() > that.max.getMonth()) {
            ...
            if (selectedCell.otherMonth) {
                for (var _i5 = 0; _i5 < cells.length; _i5++) {
                    if (!cells[_i5].otherMonth) {
                        selectedCell = cells[_i5];
                        break;
                    }
                }
            }
        ...
        key: '_prepareView',
        value: function _prepareView(selectedDate) {
            var that = this;
            var viewContent = [],
                dates = [],
                startYear = void 0,
            ...
            if (that.displayMode === 'year') {
                ...
                startYear = selectedDate ? selectedDate.getFullYear() : that._viewDates[0].getFullYear(), offset = startYear % 10;
        ...
        key: '_setDisplayModeListContent',
        value: function _setDisplayModeListContent(selectedDate) {
            var that = this;
            var dataSource = [];
            ...
            if (that.displayMode === 'year') {
                ...
                var startYear = that.min.getFullYear(),
                ...
                for (var _i7 = startYear; _i7 < endYear; _i7++) {
                    dataSource.push({ label: _i7.toString(), value: new Date(_i7, selectedDate.getMonth(), selectedDate.getDate()).toDateString() });
                }
        ...
        */
        ...
        key: '_isDateInView',
        value: function _isDateInView(date) {
            var that = this;
            ...
            for (var d = 0; d < that._viewDates.length; d++) {
                if (date.getFullYear() === that._viewDates[d].getFullYear() && date.getMonth() === that._viewDates[d].getMonth()) {
                    var cell = that._getCellByDate(date, that._animationStarted ? that.$.nextMonthsContainer : that.$.monthsContainer);
        ...
        */
        ...
        key: '_listViewContainerChangeEventHandler',
        value: function _listViewContainerChangeEventHandler(event) {
            event.stopPropagation();
        }
        ...
        key: '_listViewContainerKeyDownHandler',
        value: function _listViewContainerKeyDownHandler(event) {
            var that = this;
        ...
        key: '_showDateView',
        value: function _showDateView(newDisplayMode) {
            var that = this;
            ...
            if (arguments[0] && that.displayModeView === 'list' && that.displayMode !== 'month' && that.$.listViewContainer.items.length > 0) {
                ...
                if (target.offsetHeight) {
                    ...
                    that.$.animationPanel.style.transformOrigin = that.$.listViewContainer.offsetLeft + that.$.listViewContainer.offsetWidth / 2 + 'px ' + (target.dataItem.top > 0 ? target.offsetTop - target.offsetHeight / 2 : that.$.listViewContainer.offsetTop + that.$.listViewContainer.offsetHeight / 2) + 'px';
                }
            } else {
                if (!that._focusedCell) {
                    that._focusCell();
        ...
        */
        ...
        value: function _mouseEventsHandler(event) {
            ...
            var target = that.enableShadowDOM ? event.composedPath()[0] : event.target;
            ...
            if (that.tooltip) {
                if (cell.hasAttribute('important')) {
                    if (event.type === 'mouseover') {
                        that.$.tooltip.open();
                        that.tooltipTemplate ? that.$.tooltip.value = cell.value + '' : that.$.tooltip.innerHTML = that.$.tooltip.innerHTML.trim().length === 0 ? 'Important day!' : that.$.tooltip.innerHTML;
        ...
        value: function _moveHandler(event) {
            ...
            var details = { x: Math.round(event.pageX), y: Math.round(event.pageY) };
            var step = void 0;
            ...
            var animationTarget = void 0;

            if (that.displayMode !== 'month') {
                that.$nextMonthsContainer.addClass('jqx-calendar-date-view-container');
                animationTarget = that.$.dateViewContainer;
                if (!(that.$.nextMonthsContainer.children[0].value instanceof Date) || that.$.nextMonthsContainer.children[1].value.getFullYear() !== navigationDate.getFullYear()) {
                    that._setDisplayModeContent(navigationDate, that.$.nextMonthsContainer);
                }
            } else {
                ...
                if (!that.$.nextMonthsContainer.children[0]._date || that.$.nextMonthsContainer.children[0]._date.getTime() !== navigationDate.getTime()) {
                    var nextMonths = that.$.nextMonthsContainer.children;

                    for (var i = 0; i < nextMonths.length; i++) {
            ...
            var direction = that.scrollButtonsNavigationMode === 'portrait' ? 'y' : 'x';
        ...
        key: '_getNextDate',
        value: function _getNextDate(step) {
            var that = this;
            var targetDate = void 0;

            if (that.displayMode !== 'month') {
                var coefficient = void 0;
                var focusedDateYear = that._focusedCell && that._focusedCell.value.getFullYear();

                if (that.displayModeView === 'list') {
                    targetDate = new Date(that.$.listViewContainer.items[0].value);
                } else {
                    var container = that._animationStarted ? that.$.nextMonthsContainer : that.$.dateViewContainer;
                ...
                var dateInView = that._viewDates[0];
        ...
        */
        ...
        key: '_navigateDays',
        value: function _navigateDays(event) {
            var that = this;
            var iterations = event.key === 'ArrowUp' || event.key === 'ArrowDown' ? 7 : 1,
                ...
                case 'End':
                    {
                        ...
                        var targetDate = void 0,
                            getTargetMonthDay = function getTargetMonthDay(days) {
                            if (event.key === 'Home') {
                                ...
                                for (var _d2 = days.length - 1; _d2 >= 0; _d2--) {
                                    if (!days[_d2].otherMonth && (!days[_d2].disabled || !days[_d2].restricted)) {
                                        return days[_d2];
                                    }
                                }
                            }
        ...
        value: function _handleArrowKeys(event, iterations, coefficient) {
            var that = this;
            var targetDate = void 0;
            ...
            if (that.displayMode !== 'month') {
                var items = that._animationStarted ? that.$.nextMonthsContainer.children : that.$.dateViewContainer.children;

                iterations = event.key === 'ArrowUp' || event.key === 'ArrowDown' ? 4 : 1;

                if (that.displayMode === 'year') {
                    var targetMonth = that._focusedCell.value.getMonth() + coefficient * iterations;
                    var lastDayOfMonth = new Date(that._focusedCell.value.getFullYear(), targetMonth + 1, 0);
                    ...
                    targetDate = new Date(that._focusedCell.value.getFullYear() + coefficient * iterations, that._focusedCell.value.getMonth(), that._focusedCell.value.getDate());
                }

                if (that.displayMode === 'year') {
                    if (targetDate.getFullYear() < that.min.getFullYear() || targetDate.getFullYear() === that.min.getFullYear() && targetDate.getMonth() < that.min.getMonth() || targetDate.getFullYear() > that.max.getFullYear() || targetDate.getFullYear() === that.max.getFullYear() && targetDate.getMonth() > that.max.getMonth()) {
                        return;
                    }
                    ...
                    if (targetDate.getFullYear() < items[1].value.getFullYear() || targetDate.getFullYear() > items[10].value.getFullYear()) {
                        var focusedDate = that._focusedCell;
            ...
            var restrictedDates = that.restrictedDates.map(Number);

            targetDate = new Date(that._focusedCell.value.getFullYear(), that._focusedCell.value.getMonth(), that._focusedCell.value.getDate() + coefficient * iterations, 0, 0, 0, 0);
            ...
            var cell = that._getCellByDate(targetDate, that._animationStarted ? that.$.nextMonthsContainer : undefined);
        ...
        key: '_handlePageUpDownKeys',
        value: function _handlePageUpDownKeys(event, coefficient) {
            var that = this;
        ...
        */
        ...
        key: '_selectDate',
        value: function _selectDate(date, eventPrevented, noPropSynch) {
            var that = this;
            ...
            var selectedDates = that.selectedDates.slice(0);
        ...
        value: function _selectCell(cell) {
            ...
            if (!cell || that.restrictedDates.map(Number).indexOf(cell.value.getTime()) > -1 || that.disableAutoNavigation && false === cell.otherMonth || cell.value.getTime() < that.min.getTime() || cell.value.getTime() > that.max.getTime()) {
                return;
            }

            var index = that._selectedCells.indexOf(cell);
        ...
        */
        ...
        value: function _refreshDayOfWeekTitles() {
            var that = this;
            var days = [],
                date = new Date(),
                dayObject = void 0;
            ...
            for (var m = 0; m < months.length; m++) {
                ...
                if (that.rightToLeft) {
                    for (var d = dayNameHolders.length - 1; d >= 0; d--) {
                        dayNameHolders[d].textContent = days[counter++].name;
                        counter = counter > 6 ? 0 : counter;
                    }
                } else {
                    for (var _d3 = 1; _d3 < dayNameHolders.length; _d3++) {
                        dayNameHolders[_d3].textContent = days[counter++].name;
                        counter = counter > 6 ? 0 : counter;
                    }
                }
            }
            ...
            if (that.hasAnimation) {
                var weekTitles = that.$.monthsContainer.getElementsByClassName('jqx-calendar-week-titles')[0].innerHTML,
                    nextMonthWeekTitleContainers = that.$.nextMonthsContainer.getElementsByClassName('jqx-calendar-week-titles');

                for (var _i8 = 0; _i8 < nextMonthWeekTitleContainers.length; _i8++) {
                    nextMonthWeekTitleContainers[_i8].innerHTML = weekTitles;
                }
            }
        }
        ...
        key: '_refreshCalendarMode',
        value: function _refreshCalendarMode() {
            var that = this;
            ...
            if (that.calendarMode === 'default') {
                ...
                var sortedDates = that._viewDates.slice(0).sort(function (a, b) {
                ...
                var date = firstDateInView.toLocaleDateString(that.locale, { year: that.yearFormat, month: that.monthNameFormat });
        ...
        value: function _refreshFooter() {
            ...
            function preFormatDateOutput() {
                ...
                if (that.selectedDates.length === 1) {
                    ...
                    var sortedDates = that.selectedDates.slice(0).sort(function (a, b) {
                        return a.getTime() - b.getTime();
                    });

                    return sortedDates[0].toLocaleDateString(that.locale, { year: that.yearFormat, weekday: 'short', month: 'short', day: 'numeric' }) + ' - ' + sortedDates[sortedDates.length - 1].toLocaleDateString(that.locale, { year: that.yearFormat, weekday: 'short', month: 'short', day: 'numeric' });
                }
            }
            ...
            if (that.footerTemplate) {
                ...
                var layoutTemplate = that._layoutTemplates.filter(function (template) {
                    return template.selector === that.$.footer;
                })[0],
                    clone = document.importNode(layoutTemplate.template.content, true);

                if (layoutTemplate.bindings.length > 0) {
                    var bindingHolderElement = clone.childNodes[layoutTemplate.bindings[0].childNodeIndex];

                    layoutTemplate.selector.childNodes[layoutTemplate.bindings[0].childNodeIndex].innerHTML = bindingHolderElement.innerHTML.replace(layoutTemplate.bindings[0].bindingString, preFormatDateOutput());
                }
        ...
        */
        ...
        key: '_refreshImportantDates',
        value: function _refreshImportantDates() {
            var that = this;
            ...
            for (var d = 0; d < that.importantDates.length; d++) {
                that._setImportantDate(that.importantDates[d]);
            }
        }
    }, {
        key: '_clearImportantDates',
        value: function _clearImportantDates() {
            var that = this;
        ...
        key: '_setImportantDate',
        value: function _setImportantDate(date, cell) {
            var that = this;
            var cellByDate = that._getCellByDate(date);
        ...
        key: '_applyImportantDateTemplate',
        value: function _applyImportantDateTemplate(cell) {
            var that = this;
            ...
            var clone = document.importNode(that._importantDatesTemplate.content, true);

            if (that._bindingDetails && that._bindingDetails.length > 0) {
                var bindingHolderElement = clone.childNodes[that._bindingDetails[0].childNodeIndex];
        ...
        */
        ...
        key: '_setMonth',
        value: function _setMonth(date, monthSelector, updateDatesOnly) {
            var that = this,
            ...
            var firstDayOfWeek = (date.getDay() - that.firstDayOfWeek + 7) % 7;

            date.setDate(0);

            var previusMonthDays = date.getDate();
            ...
            if (that._selectedCells) {
                for (var i = 0; i < that._selectedCells.length; i++) {
        ...
        */
        ...
        key: '_setMonthContent',
        value: function _setMonthContent(date, month, details) {
            var that = this;
            var daysInMonth = 32 - date.getDate(),
            ...
            //Set month name
            if (that.months > 1) {
                month.firstElementChild.innerHTML = that.dateFormatFunction ? that.dateFormatFunction(month._date) : that._monthsNames[monthNumber] + ' ' + month._date.toLocaleDateString(that.locale, { year: that.yearFormat });
            }
        ...
        */
        ...
        value: function _refreshMonthNames() {
            var that = this;
            var date = new Date(),
                formatedMonth = void 0;

            date.setDate(1);
            that._monthsNames = [];

            function formatMonthName(date) {
                var result = new Intl.DateTimeFormat(that.locale, { month: that.monthNameFormat }).format(date);
            ...
            if (that.calendarMode === 'classic' && that.$.monthElement) {
                ...
                if (that._viewDates) {
                    for (var _m = 0; _m < months.length; _m++) {
                        months[_m].firstElementChild.textContent = that._monthsNames[months[_m]._date.getMonth()] + ' ' + months[_m]._date.getFullYear();
                    }

                    that._changeEventFired = 1; //ListBox change event fired, again

                    if (that.enableShadowDOM) {
                        that._setMonthElementPropertyInShadowDOM('selectedIndexes', that._viewDates.map(function (date) {
                            return date.getMonth();
                        }));
                    } else {
                        that.$.monthElement.selectedIndexes = that._viewDates.map(function (date) {
                            return date.getMonth();
                        });
                    }
                }
        ...
        */
        ...
        key: '_setMonthWeekNumbers',
        value: function _setMonthWeekNumbers(month) {
            var that = this,
                isWeekVisible = function isWeekVisible(week) {
                for (var d = 1; d < week.children.length; d++) {
            ...
            for (var w = 0; w < weeks.length; w++) {
                ...
                var cell = weeks[w].children[1];
                var monthOffset = cell.otherMonth ? cell.value.getTime() > month._date.getTime() ? 1 : -1 : 0;

                weeks[w].children[0].innerHTML = that._getWeekNumber(new Date(month._date.getFullYear(), month._date.getMonth() + monthOffset, cell.value.getDate()));
            }
        }
        ...
        */
        ...
        value: function _setRestrictedDates(restrictedDates) {
            var that = this;
            var months = that.$.monthsContainer.children;
            ...
            if (that._viewDates) {
                var isDateRestricted = function isDateRestricted(date) {
                    for (var i = 0; i < that._viewDates.length; i++) {
                ...
                for (var i = 0; i < restrictedDates.length; i++) {
                    if (isDateRestricted(restrictedDates[i])) {
                        var cell = that._getCellByDate(restrictedDates[i]);
        ...
        key: '_setScrollButtonsNavigationMode',
        value: function _setScrollButtonsNavigationMode() {
            var that = this;
        ...
        key: '_tooltipEventHandler',
        value: function _tooltipEventHandler(event) {
            var that = this;
        ...
        key: '_refreshTitle',
        value: function _refreshTitle() {
            var that = this;

            var date = that.selectedDates.length > 0 ? that.selectedDates[that.selectedDates.length - 1] : new Date();
            ...
            if (that.titleTemplate) {
                ...
                var layoutTemplate = that._layoutTemplates.filter(function (template) {
                    return template.selector === that.$.title;
                })[0],
                    clone = document.importNode(layoutTemplate.template.content, true);
                ...
                if (layoutTemplate.bindings.length > 0) {
                    ...
                    layoutTemplate.selector.childNodes[layoutTemplate.bindings[0].childNodeIndex].innerHTML = bindingHolderElement.innerHTML.replace(layoutTemplate.bindings[0].bindingString, date);
                    return;
                }
            }

            that.$.title.children[0].innerHTML = date.toLocaleDateString(that.locale, { year: that.yearFormat });
            that.$.title.children[1].innerHTML = that.view === 'landscape' ? date.toLocaleDateString(that.locale, { weekday: 'short' }) + ',' + '</br>' + date.toLocaleDateString(that.locale, { month: 'short', day: 'numeric' }) : date.toLocaleDateString(that.locale, { weekday: 'short' }) + ', ' + date.toLocaleDateString(that.locale, { month: 'short', day: 'numeric' });
        }
        ...
        */
        ...
        key: '_setWeeksVisibility',
        value: function _setWeeksVisibility(monthContainer) {
            var that = this,
                months = monthContainer.children;
            var counter = void 0;
            ...
            function setVisibility(week) {
                var shouldWeekBeHidden = function shouldWeekBeHidden(week) {
                    for (var d = 1; d < week.children.length; d++) {
                        if (!week.children[d].classList.contains('jqx-visibility-hidden')) {
                            return false;
                        }
            ...
            var _loop = function _loop(m) {
                ...
                var selectedDates = that._selectedCells.filter(function (date) {
                    return date.closest('.jqx-calendar-month') === months[m];
                });

                if (that.weeks === 6) {
                    weeks.map(function (week) {
                        return week.classList.remove('jqx-hidden');
                    });
                    return 'continue';
                }
                ...
                if (selectedDates.length > 0) {
                    var selectedDay = selectedDates[selectedDates.length - 1],
                        selectedDayWeekIndex = weeks.indexOf(selectedDay.parentElement);

                    // Weeks after the one with the last selectedDay.
                    for (var i = selectedDayWeekIndex; i < weeks.length; i++) {
                        setVisibility(weeks[i]);
                    }

                    // Weeks before the one with the last selectedDay.
                    for (var _i9 = selectedDayWeekIndex - 1; _i9 >= 0; _i9--) {
                        setVisibility(weeks[_i9]);
                    }
                } else {
                    //No selectedDay, start from 1st week
                    for (var _i10 = 0; _i10 < weeks.length; _i10++) {
                        setVisibility(weeks[_i10]);
                    }
                }
            };
        ...
        */
        ...
        value: function _refreshHeaderElements() {
            var that = this,
            ...
            var lastDateInView = that._viewDates[that._viewDates.length - 1],
                yearsInView = that._viewDates.map(function (date) {
            ...
            if (that.$.previousMonthButton && !that.disabled) {
                ...
                that.$.previousMonthButton.disabled = nextYear < that.min.getFullYear() || nextYear === that.min.getFullYear() && nextDate < that.min.getMonth() ? true : false;
            ...
            if (that.$.nextMonthButton && !that.disabled) {
                ...
                that.$.nextMonthButton.disabled = nextYear > that.max.getFullYear() || nextYear === that.max.getFullYear() && nextDate > that.max.getMonth() ? true : false;
            ...
            if (that.calendarMode === 'classic' && that.$.monthElement) {
                var items = that.$.monthElement.items;

                items.map(function (item) {
                    return item.disabled = false;
                });
                if (yearsInView.indexOf(that.min.getFullYear()) > -1) {
                    for (var i = 0; i < that.min.getMonth(); i++) {
                        items[i].disabled = true;
                    }
                }

                if (yearsInView.indexOf(that.max.getFullYear()) > -1) {
                    for (var _i11 = that.max.getMonth() + 1; _i11 < items.length; _i11++) {
                        items[_i11].disabled = true;
                    }
                }
            ...
            if (that.$.yearElement) {
                ...
                that.$.yearElement.disabled = lastDateInView.getFullYear() + 1 > that.max.getFullYear() && that._viewDates[0].getFullYear() - 1 < that.min.getFullYear() ? true : false;
            }
        }
        ...
        value: function _setMonthElementPropertyInShadowDOM(propertyname, value) {
            var that = this,
                monthElement = that.$.monthElement,
                monthElementListBox = that.$.monthElement.$ ? that.$.monthElement.$.listBox : undefined;

            var monthElementContext = monthElement.context;
            var monthElementListboxContext = void 0;
        ...
        */
        ...
        key: '_refreshHeaderTitle',
        value: function _refreshHeaderTitle() {
            var that = this;
            var date = void 0;
            ...
            function preFormatDate() {
                if (that.displayMode !== 'month') {
                    if (that.displayModeView === 'list' && that.$.listViewContainer.selectedValues[0]) {
                        var items = that.$.listViewContainer.items;

                        date = that.displayMode === 'year' ? new Date(that.$.listViewContainer.getItem(that.$.listViewContainer.selectedValues[0]).value).getFullYear() : new Date(items[0].value).getFullYear() + ' - ' + new Date(items[items.length - 1].value).getFullYear();
                    } else if (that.$.dateViewContainer.children[0].value) {
                        var dateViewItems = that._animationStarted ? that.$.nextMonthsContainer.children : that.$.dateViewContainer.children;

                        date = that.displayMode === 'year' ? dateViewItems[0].value.getFullYear() : dateViewItems[1].value.getFullYear() + ' - ' + dateViewItems[dateViewItems.length - 2].value.getFullYear();
                    }
            ...
            var layoutTemplate = that._layoutTemplates.filter(function (template) {
                return template.selector === that.$.header;
            })[0];
            ...
            var clone = document.importNode(layoutTemplate.template.content, true);

            if (layoutTemplate.bindings.length > 0) {
                var bindingHolderElement = clone.childNodes[layoutTemplate.bindings[0].childNodeIndex];

                layoutTemplate.selector.childNodes[layoutTemplate.bindings[0].childNodeIndex].innerHTML = bindingHolderElement.innerHTML.replace(layoutTemplate.bindings[0].bindingString, that.dateFormatFunction ? that.dateFormatFunction(that._viewDates) : preFormatDate());
            }
        }
        ...
        */
        ...
        key: '_updateWeeksVisibility',
        value: function _updateWeeksVisibility() {
            var that = this;
            var focusedWeek = that._focusedCell.parentElement;
            ...
            var monthWeeks = [].slice.call(focusedWeek.parentElement.children),
                weekIndex = monthWeeks.indexOf(focusedWeek),
                shouldWeekBeHidden = function shouldWeekBeHidden(week) {
                for (var d = 1; d < week.children.length; d++) {
                    if (!week.children[d].classList.contains('jqx-visibility-hidden')) {
                        return false;
                    }
            ...
            monthWeeks.map(function (week) {
                return week.classList.add('jqx-hidden');
            });

            for (var i = weekIndex; i < monthWeeks.length; i++) {
            ...
            if (counter < that.weeks) {
                for (var _i12 = weekIndex - 1; _i12 >= 0; --_i12) {
                    monthWeeks[_i12].classList.remove('jqx-hidden');
                    counter++;
        ...
        */
        ...
        key: '_getValidDates',
        value: function _getValidDates(dateOrDates) {
            var result = [];
            ...
            function validate(date) {
                if (date instanceof Date) {
                    ...
                    var regex = /(\d+[,-.\/]{1}\s*\d+[,-.\/]{1}\s*\d+)/;

                    var parseDateString = function parseDateString() {
            ...
            if (Array.isArray(dateOrDates)) {
                for (var i = 0; i < dateOrDates.length; i++) {
            ...
            }); //reset time, important for date comparing

            return result;
        }
    }, {
        key: '_getValidDate',
        value: function _getValidDate(date) {
            var that = this;
            var validDates = that._getValidDates(date);
        ...
        key: '_validateSelectedDates',
        value: function _validateSelectedDates(dates) {
            var that = this;
            var outOfRangeDates = [];
            ...
            var selectedDates = dates.slice(0),
                ...
                isRestrictedDate = function isRestrictedDate(date) {
                for (var d = 0; d < that.restrictedDates.length; d++) {
            ...
            if (that._viewDates) {
                outOfRangeDates.map(function (date) {
                    return that._selectDate(date);
                });
                that._focusCell();
            }
        }
        ...
        */
        ...
        key: '_validateMinMax',
        value: function _validateMinMax(propertyName, newValue, oldValue) {
            var that = this;
        ...
        key: '_setCellState',
        value: function _setCellState(cell, propertyName, value) {
            cell[propertyName] = value;

            var attributeName = JQX.Utilities.Core.toDash(propertyName);
        ...
        */
        ...
        value: function _validateTemplate(propertyName, template) {
        ...
        */
        ...
        value: function _weeksDownHandler(event, cell) {
            var that = this,
                date = cell.value;
            var neighbourMonth = void 0;
        ...
        */
        ...
        key: '_firstDayOfWeekValidator',
        value: function _firstDayOfWeekValidator(oldValue, newValue) {
        ...
        */
        ...
        key: '_weeksValidator',
        value: function _weeksValidator(oldValue, newValue) {
        ...
        */
        ...
        key: '_monthsValidator',
        value: function _monthsValidator(oldValue, newValue) {
        ...
        */
        ...
        key: 'styleUrls',
        get: function get() {
            return ['jqx.button.css', 'jqx.dropdownlist.css', 'jqx.dropdown.css', 'jqx.tooltip.css', 'jqx.calendar.css'];
        }
    }, {
        key: 'requires',
        get: function get() {
