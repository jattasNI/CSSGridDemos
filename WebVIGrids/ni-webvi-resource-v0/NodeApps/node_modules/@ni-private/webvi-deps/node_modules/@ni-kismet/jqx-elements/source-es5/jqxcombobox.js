...
*/
JQX('jqx-combo-box', function (_JQX$DropDownList) {
    babelHelpers.inherits(ComboBox, _JQX$DropDownList);

    function ComboBox() {
        babelHelpers.classCallCheck(this, ComboBox);
        return babelHelpers.possibleConstructorReturn(this, (ComboBox.__proto__ || Object.getPrototypeOf(ComboBox)).apply(this, arguments));
    }

    babelHelpers.createClass(ComboBox, [{
        key: 'template',
            ...
            switch (propertyName) {
                case 'autoComplete':
                    if (newValue === 'list') {
                        ...
                        //Context Fix
                        var _listBoxContext = that.$.listBox.context;

                        that.$.listBox.context = that.$.listBox;
                        that.$.listBox._filterItems(true);
                        that.$.listBox.context = _listBoxContext;
                ...
                case 'value':
                    ...
                    break;
                default:
                    babelHelpers.get(ComboBox.prototype.__proto__ || Object.getPrototypeOf(ComboBox.prototype), 'propertyChangedHandler', this).call(this, propertyName, oldValue, newValue);
                    break;
            }
        }
        ...
        */
        ...
        key: '_autoComplete',
        value: function _autoComplete(noSelectionRefresh) {
            var that = this;
            ...
            var value = that.$.input.value.length < that.minLength ? '' : that.displayMode === 'escaped' ? that._toDefaultDisplayMode(that.$.input.value) : that.$.input.value,
                //displayMode is a jqxTextBox property
            isItemFocused = function isItemFocused(items) {
            ...
            //Context Fix
            var listBoxContext = that.$.listBox.context;
        ...
        key: '_updateAutoCompleteHelper',
        value: function _updateAutoCompleteHelper() {
            var that = this;
        ...
        */
        ...
        key: '_applySelection',
        value: function _applySelection(mode, details) {
            var that = this;
            ...
            function createSelectionTags() {
                ...
                that.selectedIndexes.map(function (index) {
                ...
                that._currentSelection = that.selectedIndexes.map(function (i) {
                    return that.$.listBox._items[i][that.inputMember];
                });
            ...
            if (that.selectionMode === 'one' || that.selectionMode === 'zeroOrOne' || that.selectionMode === 'radioButton') {
                ...
                var selectionTags = that.$.selectionField.getElementsByClassName('jqx-token');

                if (that._currentSelection.length < that.selectedIndexes.length) {
                    var selectedLabels = that.selectedIndexes.map(function (index) {
                        return that.$.listBox._items[index][that.inputMember];
                    });

                    for (var i = 0; i < selectedLabels.length; i++) {
                        if (that._currentSelection.indexOf(selectedLabels[i]) < 0) {
                            var item = that.$.listBox._items[that.selectedIndexes[i]];
                            var element = void 0,
                                icon = void 0;
                    ...
                    that._positionDetection.positionDropDown();
                } else if (that._currentSelection.length > 0 && selectionTags.length === 0 || that._currentSelection.length === that.selectedIndexes.length && that._currentSelection.toString() !== that.selectedValues.toString()) {
                    createSelectionTags();
                    return;
                } else {
                    if (!details) {
                    ...
                    for (var t = 0; t < selectionTags.length; t++) {
        ...
        */
        ...
        key: '_applySelectionDisplayMode',
        value: function _applySelectionDisplayMode() {
            var that = this;
        ...
        */
        ...
        key: '_clearSelection',
        value: function _clearSelection(resetInput) {
            var that = this;
        ...
        value: function _documentDownHandler(event) {
            ...
            babelHelpers.get(ComboBox.prototype.__proto__ || Object.getPrototypeOf(ComboBox.prototype), '_documentDownHandler', this).call(this, event);
        ...
        */
        ...
        value: function _documentUpHandler(event) {
            ...
            if (target.classList.contains('jqx-drop-down-list-selection-label') && rootElement === that && that.dropDownOpenMode !== 'none') {
                that.open();

                var item = that.$.listBox._items.filter(function (item) {
                    return item[that.inputMember].toString() === target.textContent;
                })[0];
            ...
            if (that.selectionDisplayMode === 'tokens' && target.classList.contains('jqx-drop-down-list-unselect-button') && rootElement === that) {
                ...
                that.unselect(that.$.listBox._items.filter(function (item) {
                    return item[that.inputMember].toString() === target.previousElementSibling.textContent;
                })[0]);
                that.$.input.focus();
                return;
            }
            ...
            if (!that.$dropDownContainer.hasClass('jqx-visibility-hidden') && target !== 'dropDownContainer' && target !== 'item' || target === 'item' && that.selectionMode.indexOf('Many') < 0 && that.selectionMode !== 'checkBox') {
                that.close();
            }
        ...
        key: '_dropDownButtonFocusHandler',
        value: function _dropDownButtonFocusHandler(event) {
            var that = this;
        ...
        */
        ...
        value: function _inputFocusHandler(event) {
            var that = this;
            ...
            if (event.type === 'focus') {
                ...
                event.target.value !== '' || that.selectedIndexes.length !== 0 && event.target.value === '' ? that.$.container.setAttribute('has-value', '') : that.$.container.removeAttribute('has-value');

                if (!that._preventDropDownClose) {
                    if (that.opened && (that.autoComplete === 'auto' || that.autoComplete === 'inline') && that.$.input.value.length > 0 && that._focusedItem && !that._focusedItem.selected) {
        ...
        key: '_inputChangeEventHandler',
        value: function _inputChangeEventHandler(event) {
            var that = this;
        ...
        */
        ...
        value: function _keyDownHandler(event) {
            ...
            var focusedItem = typeof that._focusedItem === 'function' ? that._focusedItem() : that._focusedItem;
                ...
                case 'Backspace':
                    if (that.$.input.previousElementSibling) {
                        if (that.$.input.value.length === 0) {
                            ...
                            var itemToBeRemoved = that.$.listBox.getItem(that.$.input.previousElementSibling._value);
        ...
        key: '_autoOpenOnKeyDown',
        value: function _autoOpenOnKeyDown(event) {
            var that = this;
        ...
        */
        ...
        value: function _keyUpHandler(event) {
            ...
            if (['one', 'radioButton', 'zeroOrOne'].indexOf(that.selectionMode) > -1 && that.selectedIndexes.length === 1 && that.$.input.value !== (that._currentSelection ? that._currentSelection[0] : undefined)) {
            ...
            if (target === that.$.input && event.key.indexOf('Arrow') < 0 && ['Control', 'Shift'].indexOf(event.key) < 0) {
                ...
                if (that.autoComplete !== 'none' && that.autoComplete !== 'list') {
                    if (that.$.input.value !== (that._currentSelection ? that._currentSelection.toString() : undefined) || that.$.listBox._filteredItems && that.$.listBox._filteredItems.length !== that.$.listBox._items.length) {
                        if (that._autoCompleteTimer) {
                            clearTimeout(that._autoCompleteTimer);
                        }
        ...
        key: '_queryItems',
        value: function _queryItems(selectItem) {
            var that = this;
            ...
            var foundItems = that.$.listBox._queryItems(that.$.input.previousElementSibling ? that.$.input.value : that.value, that.incrementalSearchMode);
            ...
            if (that.autoComplete === 'list' && (that.enableShadowDOM ? that.shadowRoot.activeElement : document.activeElement) === that.$.input && that.$.input.value.length >= that.minLength) {
                that._updateAutoCompleteHelper();
            }
        }
