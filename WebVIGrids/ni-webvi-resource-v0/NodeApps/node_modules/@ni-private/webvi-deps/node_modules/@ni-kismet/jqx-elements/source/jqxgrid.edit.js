JQX.Utilities.Assign('Grid.Edit', class Edit {
    _renderCommandBar() {
        const that = this;
    ...
    _renderAddNewRow() {
        ...
        const newNearRow = new JQX.Grid.Row({ data: {}, index: -1, grid: that, freeze: 'near', visible: that.editing.addNewRow.position !== 'far', autoGenerated: true, addNewRow: true });
        const newFarRow = new JQX.Grid.Row({ data: {}, index: -2, grid: that, freeze: 'far', visible: that.editing.addNewRow.position !== 'near', autoGenerated: true, addNewRow: true });
    ...
    addUnboundRow(count, position) {
        ...
        for (let i = 0; i < count; i++) {
            const id = JQX.Utilities.Core.createGUID().replace(/-/ig, '');
            const row = new JQX.Grid.Row({ index: -1, unbound: true, id: id, grid: that });
    ...
    _insertNewRowBefore(callback, insertAtBottom) {
        const that = this;
        const row = new JQX.Grid.Row({ index: insertAtBottom ? 0 : -1, id: JQX.Utilities.Core.createGUID(), grid: that });
    ...
    _getCommandColumnCommandsTemplate() {
        ...
        for (let item in dataSource) {
            ...
            if (!isCommandSet) {
                commandColumnItem += ' item="' + item + '" command="' + properties.command + '"' + ' class="jqx-grid-command-item">';
                isCommandSet = true;
            }
            ...
            if (showIcon && showLabel) {
                commandColumnItem += '<span class="jqx-grid-icon ' + properties.icon + '"></span>';
    ...
    _getAutoCompleteCellEditor(cell) {
        ...
        if (!that._cellEditors[id]) {
            const element = document.createElement('jqx-input');
    ...
    _getDateTimePickerCellEditor(cell) {
        ...
        if (!that._cellEditors[id]) {
            const element = document.createElement('jqx-date-time-picker');
    ...
    _openAddRowDialog(row, command) {
        ...
        if (!that._dialogAddRow) {
            ...
            dialog.btnConfirm.onclick = function () {
                ...
                const row = new JQX.Grid.Row({ index: that.rows.length + index, grid: that, data: data });
