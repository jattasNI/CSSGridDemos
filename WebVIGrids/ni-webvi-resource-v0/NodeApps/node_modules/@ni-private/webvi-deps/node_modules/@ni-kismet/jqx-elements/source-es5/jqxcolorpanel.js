...
JQX('jqx-color-panel', function (_JQX$BaseElement) {
    babelHelpers.inherits(ColorPanel, _JQX$BaseElement);

    function ColorPanel() {
        babelHelpers.classCallCheck(this, ColorPanel);
        return babelHelpers.possibleConstructorReturn(this, (ColorPanel.__proto__ || Object.getPrototypeOf(ColorPanel)).apply(this, arguments));
    }

    babelHelpers.createClass(ColorPanel, [{
        key: 'template',
        ...
        */
        ...
        value: function propertyChangedHandler(propertyName, oldValue, newValue) {
            ...
            switch (propertyName) {
                ...
                case 'inverted':
                    {
                        var hueScaleThumbHeight = that.$.hueScaleThumb.offsetHeight,
                        ...
                        that.$.colorPaletteRadial.style.setProperty('--jqx-color-panel-brightness', that.$.brightnessScaleThumb.s || 0);

                        //that.$.colorPaletteRadial.style.setProperty('--jqx-color-panel-brightness', (that._getColorBrightness() || 0));
                    ...
                    if (propertyName === 'displayMode') {
                        ...
                        if (that.value !== null && newValue !== 'palette' && newValue !== 'hexagonal' && newValue !== 'radial') {
                            if (that.$.alphaChannelThumb) {
                                that.$.alphaChannelThumb.style.left = 0;
                ...
                case 'editAlphaChannel':
                    {
                        ...
                        var _previousValue = that.value;

                        that.value = that._getValue(that.valueFormat);

                        if (that.value !== _previousValue) {
                            that.$.fireEvent('change', {
                ...
                case 'columnCount':
                    {
                        var style = that.$.container.style;

                        newValue === null || isNaN(newValue) ? style.removeProperty('--jqx-color-panel-grid-mode-column-count') : style.setProperty('--jqx-color-panel-grid-mode-column-count', newValue);
                ...
                case 'tooltipDisplayMode':
                    ...
                    break;
                default:
                    babelHelpers.get(ColorPanel.prototype.__proto__ || Object.getPrototypeOf(ColorPanel.prototype), 'propertyChangedHandler', this).call(this, propertyName, oldValue, newValue);

                    if (propertyName === 'disabled') {
                        var inputs = that.getElementsByClassName('color-input');
        ...
        */
        ...
        value: function ready() {
            babelHelpers.get(ColorPanel.prototype.__proto__ || Object.getPrototypeOf(ColorPanel.prototype), 'ready', this).call(this);
            ...
            if (that.columnCount === null || isNaN(that.columnCount)) {
                that.$.container.style.removeProperty('--jqx-color-panel-grid-mode-column-count');
            } else {
                that.$.container.style.setProperty('--jqx-color-panel-grid-mode-column-count', that.columnCount);
            }
        ...
        */
        ...
        key: '_createApplyValueButtons',
        value: function _createApplyValueButtons(createOnDemand) {
            var that = this;
            ...
            function createButton(type) {
                var button = document.createElement('jqx-button');
        ...
        key: '_createAlphaChannel',
        value: function _createAlphaChannel() {
            var that = this;
            ...
            var aChannel = that.$.alphaChannelContainer.querySelector('input');
        ...
        key: '_createGridModeComponents',
        value: function _createGridModeComponents() {
            var that = this;

            var nonGridModes = ['radial', 'palette', 'hexagonal'];
            ...
            that.$.materialGridSamplesContainer.$ = JQX.Utilities.Extend(that.$.materialGridSamplesContainer);
            ...
            that.$.buttonCustom = gridModeContainer.querySelector('jqx-button');
            that.$.tooltip = gridModeContainer.querySelector('jqx-tooltip');
        ...
        key: '_createColorControls',
        value: function _createColorControls(createOnDemand) {
            var that = this;
        ...
        key: '_getColorBrightness',
        value: function _getColorBrightness() {
            var rgb = this._RGBA;
            ...
            return Math.min(255, Math.sqrt(rgb.r * rgb.r * .241 + rgb.g * rgb.g * .691 + rgb.b * rgb.b * .068));
        }
        ...
        */
        ...
        value: function _containerClickHandler(event) {
            ...
            var isButton = event.target.closest('jqx-button');
            ...
            if (event.target === that.querySelector('.preview-container-top') && that.applyValueMode === 'useButtons' && !that.disableUndo) {
                that._applyValue(that.value);
            }
        }
        ...
        value: function _reset() {
            var oldValue = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.value;
            ...
            if (that.displayMode !== 'hexagonal' && that.displayMode !== 'radial' && that.displayMode !== 'palette') {
                ...
                //if (that.displayMode === 'radial') {
                //    that.$.colorPaletteRadial.style.setProperty('--jqx-color-panel-brightness', 0);
                //}
                //else if (that.displayMode === 'hexagonal') {
                //    that._generateHoneycomb(that.columnCount, that.inverted ? { s: that._HSV.s } : { v: that._HSV.v });
                //}
        ...
        */
        ...
        key: '_handleColorPaletteDown',
        value: function _handleColorPaletteDown(event) {
            var that = this,
            ...
            var activeThumb = that.displayMode === 'palette' || that.displayMode === 'grid' ? that.$.colorPaletteThumb : that.$.colorPaletteRadialThumb;
            ...
            if (that.displayMode === 'palette' || that.$.container.className.indexOf('custom-color-selection') > -1) {
                ...
                var diameter = that.$.colorPaletteRadial.offsetWidth,
                ...
                var brightnessScaleThumb = that.$.brightnessScaleThumb,
                    brightnessScale = that.$.brightnessScale,
                    value = that._validateInRange(2 * that._calcSatDistance(dx, dy) / diameter, 0, 1),
                    brightness = that._validateInRange(brightnessScaleThumb.offsetTop - brightnessScaleThumb.offsetHeight / 2, 0, brightnessScale.offsetHeight - brightnessScaleThumb.offsetHeight) / brightnessScale.offsetHeight;
        ...
        */
        ...
        key: '_handleScalesDown',
        value: function _handleScalesDown(event) {
            var that = this,
                coord = that._getXYcoordinates(event);
            var thumbCenter = void 0,
                thumbMaxRange = void 0;
            ...
            if (coord.container === that.$.hueScale) {
                ...
                thumbCenter = coord.x - that.$.alphaChannelThumb.offsetWidth / 2;
                thumbMaxRange = that.$.alphaChannelScale.offsetWidth - that.$.alphaChannelThumb.offsetWidth;
                that.$.alphaChannelThumb.style.left = 100 * that._validateInRange(thumbCenter, 0, thumbMaxRange) / that.$.alphaChannelScale.offsetWidth + '%';
                that.$.alphaChannelThumb.a = that._RGBA.a = (coord.container.offsetWidth - coord.x) / coord.container.offsetWidth;
                that._applyAlphaValue(that._RGBA.a, 'alphaScale');
            }
        ...
        key: '_updateBrightnessScale',
        value: function _updateBrightnessScale(coord) {
            var that = this;

            var thumbCenter = coord.y - that.$.brightnessScaleThumb.offsetHeight / 2,
            ...
            that.$.colorPaletteRadial.style.setProperty('--jqx-color-panel-brightness', brightness);
        }
        ...
        key: '_updateHueScale',
        value: function _updateHueScale(coord) {
            var that = this,
        ...
        */
        ...
        key: '_moveThumbHandler',
        value: function _moveThumbHandler(event, customColorSelect) {
            var that = this,
            ...
            var coord = that._getXYcoordinates(event);

            if (!coord) {
                return;
            }

            var containerWidth = that._activeComponent.offsetWidth,
            ...
            switch (that._activeComponent) {
                ...
                case that.$.colorPaletteRadial:
                    {
                        ...
                        var previewContainerBottom = that.querySelector('.preview-container-bottom');
                ...
                case that.$.brightnessScale:
                    {
                        ...
                        if (that.displayMode === 'hexagonal') {
                            var customHSV = that.inverted ? { s: that._HSV.s } : { v: that._HSV.v };
                            that._generateHoneycomb(that.columnCount, customHSV);
                        }
                ...
                case that.$.alphaChannelScale:
                    {
                        var thumbCenter = coord.x - that.$.alphaChannelThumb.offsetWidth / 2,
                            thumbMaxRange = containerWidth - that.$.alphaChannelThumb.offsetWidth;

                        that.$.alphaChannelThumb.a = that._RGBA.a = (containerWidth - coord.x) / containerWidth;
                        that.$.alphaChannelThumb.style.left = 100 * that._validateInRange(thumbCenter, 0, thumbMaxRange) / containerWidth + '%';
        ...
        */
        ...
        key: '_releaseThumbUpHandler',
        value: function _releaseThumbUpHandler() {
            var that = this,
        ...
        */
        ...
        value: function _handleCancelButtonClick(fireClickEvent) {
            var that = this,
                oldValue = that.value;
            ...
            if (that._customColorSelection === false) {
                ...
                var container = that.displayMode === 'hexagonal' ? that.$.colorPaletteHexagonal : that.$.gridModeContainer;
                var selectedColor = container.getElementsByClassName('selected');
            ...
            if (that.displayMode === 'palette' || that.displayMode === 'radial') {
                var previewContainerTop = that.querySelector('.preview-container-top');
        ...
        */
        ...
        key: '_handleOkButtonClick',
        value: function _handleOkButtonClick(fireClickEvent) {
            var that = this,
                oldValue = that.value;
            ...
            if (that._customColorSelection === false || typeof that._customColorSelection === 'undefined') {
                ...
                if (that.displayMode !== 'palette' && that.displayMode !== 'radial' && that.displayMode !== 'hexagonal' && (!that._selectedItem || !that._selectedItem.classList.contains('selected'))) {
                    that.value = null;
                } else {
                    that.value = that._getValue(that.valueFormat);
                }
                ...
                if (that.value === null) {
                    if (that.displayMode === 'palette') {
                        ...
                        // Unselect selected colors
                        var selectedColor = that.$.gridModeContainer.getElementsByClassName('selected');
                ...
                var customColor = that._getValue(that.valueFormat);
        ...
        */
        ...
        value: function _handleInputChange(event) {
            ...
            var isRGBInput = ['r-channel', 'g-channel', 'b-channel'].find(function (className) {
            ...
            if (isRGBInput || isAlphaChannelInput) {
                ...
                var _oldValue2 = that.value ? that._getValue('hex') : null,
                ...
                if (that._isHEX(testValue)) {
                    ...
                    target.value = _oldValue2 ? _oldValue2.substring(1) : 'ffffff';
                }
            }
        ...
        */
        ...
        key: '_applyValue',
        value: function _applyValue(value, event) {
            var that = this;
            ...
            if (that.displayMode === 'palette' || that.$.container.className.indexOf('custom-color-selection') > -1) {
                ...
                //that._applyValueInAnyGridMode(value, that.$.colorPaletteHexagonal);
            } else {
        ...
        */
        ...
        key: '_applyAlphaValue',
        value: function _applyAlphaValue(value, vendor) {
            var that = this;
            ...
            if (that.$.alphaChannelScale) {
                if (vendor !== 'alphaScale') {
                    var thumbWidth = that.$.alphaChannelThumb.offsetWidth,
                        scaleWidth = that.$.alphaChannelScale.offsetWidth,
                        offset = (1 - value) * scaleWidth > thumbWidth ? (1 - value) * thumbWidth : 0;

                    that.$.alphaChannelThumb.style.left = (1 - value) * scaleWidth - offset + 'px';
                }

                if (vendor !== 'alphaInput') {
                    var aInput = that.$.alphaChannelContainer.querySelector('input.a-channel');
        ...
        */
        ...
        key: '_applyValueInPaletteMode',
        value: function _applyValueInPaletteMode(event) {
            var that = this,
                HEXvalue = that._getValue('hex'),
                scaleHeight = that.$.hueScale.offsetHeight,
                thumbHeight = that.$.hueScaleThumb.offsetHeight,
                hsvYoffsetinPX = that._HSV.h ? that.$.hueScale.offsetHeight * (360 - that._HSV.h) / 360 : 0,
                offset = hsvYoffsetinPX - thumbHeight > 0 ? hsvYoffsetinPX / scaleHeight * thumbHeight : 0,
                hexInput = that.querySelector('input.hex');
            ...
            that.$.alphaChannelContainer.style.setProperty('--jqx-color-panel-alpha-channel-color', HEXvalue);

            if (!that._activeComponent) {
                if (that.$.alphaChannelThumb) {
                    that.$.alphaChannelThumb.style.left = 100 * (1 - that._RGBA.a) + '%';
                }

                that.$.hueScaleThumb.style.top = Math.min(that.$.hueScale.offsetHeight - that.$.hueScaleThumb.offsetHeight, hsvYoffsetinPX + offset) + 'px';
        ...
        */
        ...
        key: '_applyValueInRadialMode',
        value: function _applyValueInRadialMode(event) {
            var that = this,
                HEXvalue = that._getValue('hex'),
                hexInput = that.querySelector('input.hex');
            ...
            if (!that._activeComponent) {
                var thumbMaxRange = that.$.brightnessScale.offsetHeight - that.$.brightnessScaleThumb.offsetHeight;

                if (that.$.alphaChannelThumb) {
                    that.$.alphaChannelThumb.style.left = 100 * (1 - that._RGBA.a) + '%';
                }

                that.$.brightnessScaleThumb.style.top = thumbMaxRange * (that.inverted ? that._HSV.v : 1 - that._HSV.v) + 'px';
            }
            ...
            if (that.inverted) {
                var color1 = that.value === null ? 'black' : that._HSVtoRGBA({ h: that._HSV.h, s: 1, v: that._HSV.v });
                that.$.brightnessScale.style.backgroundImage = 'linear-gradient(' + color1 + ', white)';
            } else {
                var _color = that.value === null ? 'white' : that._HSVtoRGBA({ h: that._HSV.h, s: that._HSV.s, v: 1 });
                that.$.brightnessScale.style.backgroundImage = 'linear-gradient(' + _color + ', black)';
            }
        }
        ...
        */
        ...
        key: '_applyValueInHexagonalMode',
        value: function _applyValueInHexagonalMode() {
            var that = this,
                HEXvalue = that._getValue('hex'),
                hexInput = that.querySelector('input.hex');
            ...
            var rgbaTempValue = that._getValue('rgba');

            that._updatePreviewContainerValues(rgbaTempValue ? rgbaTempValue : that._getValue('hex'));

            if (that.inverted) {
                var color1 = that._HSVtoRGBA({ h: that._HSV.h, s: 1, v: that._HSV.v }, 1);
                that.$.brightnessScale.style.backgroundImage = 'linear-gradient(' + color1 + ', white)';
            } else {
                var _color2 = that._HSVtoRGBA({ h: that._HSV.h, s: that._HSV.s, v: 1 }, 1);
                that.$.brightnessScale.style.backgroundImage = 'linear-gradient(' + _color2 + ', black)';
            }
        ...
        */
        ...
        key: '_radialThumbPoint',
        value: function _radialThumbPoint(x, y, angle, distance) {
            var point = {
        ...
        */
        ...
        key: '_updatePreviewContainerValues',
        value: function _updatePreviewContainerValues(newValue, okClick) {
            var that = this,
                previewContainer = that.querySelector('.preview-container');
        ...
        */
        ...
        key: '_applyValueInAnyGridMode',
        value: function _applyValueInAnyGridMode(value, container) {
            var that = this;
            ...
            var samples = Array.from(container.getElementsByClassName('color-sample'));
            var match = that._targetItem;
        ...
        */
        ...
        key: '_generatePaletteGrid',
        value: function _generatePaletteGrid() {
            var that = this;
            ...
            var palette = that['_' + that.palette + 'Palette'];
        ...
        */
        ...
        key: '_generatePaletteUserColorGrid',
        value: function _generatePaletteUserColorGrid(noSelection) {
            var that = this;
            ...
            var palette = that._userPalette.filter(function (v) {
                return v !== '';
            }).slice(0, that.columnCount || 1),
                fragment = document.createDocumentFragment();
            var newColor = void 0;

            palette = [].concat(babelHelpers.toConsumableArray(new Set(palette)));

            for (var i = 0; i < palette.length; i++) {
                if (palette[i]) {
                    var colorItem = that._createColorSample(palette[i], 'user-color');
            ...
            if (!noSelection && newColor.colorCode) {
        ...
        */
        ...
        key: '_initializeBaseVars',
        value: function _initializeBaseVars() {
            var that = this;
            ...
            that._defaultModeThemeColors = that.gridThemeColors === null ? ['#FFFFFF', '#000000', '#E6E6E6', '#495469', '#5671C2', '#D48439', '#A5A5A5', '#EEC328', '#7399D3', '#85AA4C'] : that.gridThemeColors;

            that._defaultModeStandardColors = that.gridStandardColors === null ? ['#A52A0D', '#DB3A15', '#EEC328', '#FEFE33', '#A6CD57', '#62AC54', '#65ADEE', '#3F6FBE', '#10205F', '#64379E'] : that.gridStandardColors;

            that._defaultModeShadesColors = that.gridShadeColors === null ? ['#F2F2F2', '#808080', '#D0CECE', '#D6DCE4', '#DDEBF7', '#FCE4D6', '#EBEBEB', '#FFF2CC', '#DDE5F7', '#E2EFDA', '#D8D8D8', '#595959', '#AEAAAA', '#ACB9CA', '#BDD7EE', '#F6CAAD', '#DBDBDB', '#FFE699', '#B4C6E7', '#C6E0B4', '#BFBFBF', '#404040', '#757171', '#8497B0', '#9BC2E6', '#F4B084', '#C0C0C0', '#FFD966', '#8EA9DB', '#A9D08E', '#A6A6A6', '#262626', '#312F2F', '#333F4F', '#2F75B5', '#C65911', '#7B7B7B', '#BF8F00', '#305496', '#548235', '#808080', '#0D0D0D', '#161616', '#222B35', '#1F4E78', '#833C0C', '#525252', '#806000', '#203764', '#375623'] : that.gridShadeColors;
        ...
        */
        ...
        key: '_mapDataSourceToPalette',
        value: function _mapDataSourceToPalette() {
            var that = this;
            var customPalette = [];
            ...
            for (var i = 0; i < that.paletteColors.length; i++) {
                var item = that.paletteColors[i];

                if (typeof item === 'string') {
                    customPalette.push(that.paletteColors[i]);
                } else if ((typeof item === 'undefined' ? 'undefined' : babelHelpers.typeof(item)) === 'object' && (item.value && typeof item === 'string' || item[that.valueMember] && typeof item[that.valueMember] === 'string')) {
                    customPalette.push(that.paletteColors[i]);
                } else {
                    customPalette.push(that.paletteColors[i]);
                }
        ...
        */
        ...
        key: '_handleSampleClick',
        value: function _handleSampleClick(event) {
            var that = this,
                item = event.target,
                oldValue = that.value;
            var itemColor = item.colorCode;
        ...
        */
        ...
        key: '_changeSampleSelection',
        value: function _changeSampleSelection(item) {
            var that = this;
            ...
            var strokeColor = that._getContrastColor(that._rgbaToHEX(that._toRGBA(item.colorCode))),
                svg = item.closest('svg');

            Array.from(that.getElementsByClassName('selected')).forEach(function (item) {
        ...
        */
        ...
        key: '_getXYcoordinates',
        value: function _getXYcoordinates(event) {
            if (!event) {
                return;
            }
            ...
            var parent = target.closest('.color-palette') || target.closest('.color-palette-radial') || target.closest('.hue-scale') || target.closest('.brightness-scale') || target.closest('.alpha-channel-scale');
            ...
            var offsetX = event.pageX - window.pageXOffset - parentRect.left,
        ...
        */
        ...
        key: '_paletteDownHandler',
        value: function _paletteDownHandler(event) {
            var that = this;

            if (!JQX.Utilities.Core.isMobile && event.button !== 0 || that.disabled || that.readonly) {
                return;
            }

            that._dragDetails = { x: event.pageX - window.pageXOffset, y: event.pageY - window.pageYOffset };

            var target = (event.originalEvent || event).target;
        ...
        */
        ...
        key: '_setPalettePointerXYPosition',
        value: function _setPalettePointerXYPosition(event) {
            var that = this;
            var coord = void 0;
            ...
            if (that.displayMode === 'palette' || that.$.container.className.indexOf('custom-color-selection') > -1) {
                ...
                if (event) {
                    ...
                    var offsetW = coord.container.offsetWidth ? coord.container.offsetWidth : that.$.colorPaletteRadial.offsetWidth,
                        offsetH = coord.container.offsetHeight ? coord.container.offsetHeight : that.$.colorPaletteRadial.offsetHeight;

                    var paletteCoordX = Math.round(Math.cos(that._HSV.h * Math.PI / 180) * offsetW / 2 + offsetW / 2),
                    ...
                    var thumb = that._radialThumbPoint(that.$.colorPaletteRadial.offsetWidth / 2, that.$.colorPaletteRadial.offsetHeight / 2, that._HSV.h, that._HSV.s);
        ...
        */
        ...
        key: '_handleGridModeCellHover',
        value: function _handleGridModeCellHover(event) {
            var that = this,
            ...
            that.$.tooltip.arrow = that.displayMode === 'hexagonal' ? false : true;
            ...
            var itemColor = targetCell.h && targetCell.s && targetCell.v ? that._HSVtoRGBA({ h: targetCell.h, s: targetCell.s, v: targetCell.v }, that._RGBA.a) : that._toRGBA(targetCell.colorCode);
        ...
        */
        ...
        key: '_localizeLabels',
        value: function _localizeLabels() {
            var that = this,
                rLabel = that.querySelector('label.r-channel'),
                gLabel = that.querySelector('label.g-channel'),
                bLabel = that.querySelector('label.b-channel'),
                aLabel = that.querySelector('label.a-channel'),
                hexLabel = that.querySelector('label.hex');
            ...
            if (that.$.defaultSamplesContainer) {
                //Theme colors label
                var themeColorsLabel = that.$.defaultSamplesContainer.querySelector('.theme-colors-label'),
        ...
        */
        ...
        key: '_generateHoneycomb',
        value: function _generateHoneycomb(cols, customHSV) {
            var that = this,
                shortStep = parseInt(getComputedStyle(that).getPropertyValue('--jqx-color-panel-grid-mode-item-size')) / 4,
            ...
            if (!that._draw) {
                that._draw = new JQX.Utilities.Draw(that.$.colorPaletteHexagonal);
            }
            ...
            if (that.$.materialGridSamplesContainer) {
                that.$.materialGridSamplesContainer.$.addClass('jqx-hidden');
            }

            for (var i = 0; i < offsets.length; i++) {
                var dx = offsets[i].x - centerElement.x,
                    dy = offsets[i].y - centerElement.y,
                    satValCoefficient = that._validateInRange(that._calcSatDistance(dx, dy) / centerElement.x, 0, 1);
                var hue = that._calcHueAngle(dx, dy) < 0 ? 360 + that._calcHueAngle(dx, dy) : that._calcHueAngle(dx, dy);
                var s = void 0,
                    v = void 0;
                ...
                var item = that._hexagon(hexagonSideSize, settings);
            ...
            that.$.container.style.setProperty('--jqx-color-panel-palette-width', hexagonSize.width + 'px');
            that.$.container.style.setProperty('--jqx-color-panel-palette-height', hexagonSize.height + 'px');
            that._applyValueInHexagonalMode();
            that._hideControlsByPriority(); // 
        }
        ...
        */
        ...
        key: '_hexagon',
        value: function _hexagon(side, settings) {
            //
            var that = this,
        ...
        */
        ...
        key: '_shortStepHexagon',
        value: function _shortStepHexagon(sideSize) {
        ...
        */
        ...
        key: '_getHoneycomb30degDimmensions',
        value: function _getHoneycomb30degDimmensions(cells, itemSideSize, offset) {
            var that = this,
        ...
        */
        ...
        key: '_generateShades',
        value: function _generateShades(items, shadesOf) {
            var that = this,
                type = shadesOf || 'gray',
                base = 255;
            var color = void 0,
            ...
            var step = 255 / (items - 1);

            for (var i = 0; i < items; i++) {
                var shadeValue = Math.max(parseInt(base - i * step), 0);
        ...
        */
        ...
        key: '_generateColorSpectrum',
        value: function _generateColorSpectrum(columns) {
            var that = this;
            var colors = [];

            columns = Math.max(parseInt(columns), 2);

            var hueStep = 360 / (columns - 1),
                svStep = 2 / columns;
            var hue = 0,
                saturation = 1,
                brightnessValue = 1;
        ...
        */
        ...
        key: '_generateSpectrumGrid',
        value: function _generateSpectrumGrid(columns) {
            var that = this;
            ...
            var spectrumItems = Math.pow(columns, 2),
                fragment = document.createDocumentFragment(),
                containers = {},
            ...
            var spectrumColors = void 0;

            containersNames.forEach(function (name) {
                containers[name] = document.createElement('div');
                containers[name].className = JQX.Utilities.Core.toDash(name);
                fragment.appendChild(containers[name]);
            });
            ...
            for (var i = 0; i < columns; i++) {
                var colorSample = that._createColorSample(grayRowShades[i], 'gray-shade');
                containers.grayShadesBox.appendChild(colorSample);
            }

            for (var _i = 0; _i < spectrumItems; _i++) {
                var _colorSample = that._createColorSample(spectrumColors[_i], 'spectrum');
                containers.spectrumBox.appendChild(_colorSample);
            }

            for (var _i2 = 0; _i2 < columns; _i2++) {
                var colorCode = customColors[_i2];

                if (!colorCode) {
                    break;
                }

                var _colorSample2 = that._createColorSample(colorCode, 'custom-color');
                containers.customColorsBox.appendChild(_colorSample2);
            }
        ...
        */
        ...
        key: '_generateDefaultGrid',
        value: function _generateDefaultGrid() {
            var that = this;
            ...
            for (var i = 0; i < that._defaultModeShadesColors.length; i++) {
                containers.themeShadesBox.appendChild(that._createColorSample(shadeColors[i], 'shades-color'));
            }

            for (var _i3 = 0; _i3 < columns; _i3++) {
                containers.themeColorsBox.appendChild(that._createColorSample(themeColors[_i3], 'theme-color'));
                containers.standardColorsBox.appendChild(that._createColorSample(standardColors[_i3], 'standard-color'));
            }
        ...
        key: '_mixColors',
        value: function _mixColors(colorA, colorB, shade) {
            var decToHex = function decToHex(d) {
                return d.toString(16);
            },
                hexToDec = function hexToDec(h) {
                return parseInt(h, 16);
            };
            var mixedColorPair = void 0,
                colorBlend = '#';
            ...
            for (var i = 0; i <= 5; i += 2) {
                var colorPair1 = hexToDec(colorA.substr(i, 2)),
        ...
        */
        ...
        key: '_generateMaterialGrid',
        value: function _generateMaterialGrid() {
            var that = this;
            ...
            // Bugfix when changing mode from materialGrid to hexagonal
            that.$.materialGridSamplesContainer.$.removeClass('jqx-hidden');

            var colorSample1 = that._createColorSample('rgb(255, 255, 255, 0)', 'material-color label-column');
            fragment.appendChild(colorSample1);
            Object.entries(that._materialColors['Red']).forEach(function (color) {
                var colorSample = that._createColorSample('rgb(255, 255, 255, 0)', 'material-color header');

                colorSample.innerText = color[0];
                fragment.appendChild(colorSample);
            });

            Object.entries(that._materialColors).forEach(function (gama) {
                var colorSample1 = that._createColorSample('rgb(255, 255, 255, 0)', 'material-color label-column');
                colorSample1.innerText = gama[0];
                fragment.appendChild(colorSample1);

                Object.entries(gama[1]).forEach(function (color) {
                    var colorSample = that._createColorSample(color[1], 'material-color');
                    fragment.appendChild(colorSample);
                });
            });
        ...
        */
        ...
        key: '_createColorSample',
        value: function _createColorSample(color, className) {
        ...
        */
        ...
        key: '_calculateHexagonCoords',
        value: function _calculateHexagonCoords(columns, elementSideSize, offset) {
            var that = this,
                rows = columns,
            ...
            for (var i = 0; i < rows; i++) {
                var rowSize = i < firstRowSize ? firstRowSize + i : columns - 1 - (i - firstRowSize),
                    x0 = i < firstRowSize ? firstRowX - i * offsetXnextRow : firstRowX - (columns - i - 1) * offsetXnextRow,
                    y0 = i * offsetYnextRow;

                for (var j = 0; j < rowSize; j++) {
                    var x = x0 + j * offsetXnextItemInRow;
        ...
        */
        ...
        key: '_generateGridStructures',
        value: function _generateGridStructures() {
            var that = this;
        ...
        key: '_generateRadialCanvas',
        value: function _generateRadialCanvas() {
            var that = this;

            if (that.displayMode !== 'radial' || that.displayMode === 'radial' && !JQX.Utilities.Core.Browser.Firefox) {
                return;
            }

            var diameter = that.$.colorPaletteRadial.offsetWidth,
                radius = diameter / 2;
            var canvas = that.$.colorPaletteRadial.querySelector('canvas');
            ...
            for (var x = -radius; x < radius; x++) {
                for (var y = -radius; y < radius; y++) {
                    var _that$_xy2polar = that._xy2polar(x, y),
                        _that$_xy2polar2 = babelHelpers.slicedToArray(_that$_xy2polar, 2),
                        r = _that$_xy2polar2[0],
                        phi = _that$_xy2polar2[1];
                    ...
                    var hue = that._rad2deg(phi),
                        saturation = 1.0,
                        value = 1.0;

                    var _that$_hsv2rgb = that._hsv2rgb(hue, saturation, value),
                        _that$_hsv2rgb2 = babelHelpers.slicedToArray(_that$_hsv2rgb, 3),
                        red = _that$_hsv2rgb2[0],
                        green = _that$_hsv2rgb2[1],
                        blue = _that$_hsv2rgb2[2];
        ...
        key: '_rad2deg',
        value: function _rad2deg(rad) {
            return (rad + Math.PI) / (2 * Math.PI) * 360;
        }
    }, {
        key: '_hsv2rgb',
        value: function _hsv2rgb(hue, saturation, value) {
        ...
        */
        ...
        key: '_hideControlsByPriority',
        value: function _hideControlsByPriority() {
            var that = this,
                colorControlsContainer = that.querySelector('.color-controls-container');
            ...
            function hideComponent(className) {
                var components = that.getElementsByClassName(className);

                for (var _i4 = 0; _i4 < components.length; _i4++) {
                    components[_i4].classList.add('jqx-hidden');
                }
            }

            var priority = that.hideContentToFit.slice().reverse(),
                controlledElements = ['hex', 'r-channel', 'g-channel', 'b-channel', 'a-channel', 'preview-container'];
            var i = priority.length;

            controlledElements.forEach(function (item) {
                //if (that['$' + item]) {
                //    that['$' + item].removeClass('jqx-hidden');
                //}

                var elements = that.getElementsByClassName(item);

                for (var e = 0; e < elements.length; e++) {
                    elements[e].classList.remove('jqx-hidden');
                }
            });
            ...
            while (colorControlsContainer.scrollHeight > colorControlsContainer.offsetHeight && i > 0) {
                ...
                switch (priority[i]) {
                    case 'HEX':
                        //controlledElements.slice(0, 2).forEach(item => that['$' + item].addClass('jqx-hidden'));
                        //controlledElements.slice(0, 2).forEach(item => that['$' + item].addClass('jqx-hidden'));
                        hideComponent(controlledElements[0]);
                        break;
                    case 'RGB':
                        //controlledElements.slice(2, 8).forEach(item => that['$' + item].addClass('jqx-hidden'));
                        controlledElements.slice(1, 4).forEach(function (className) {
                            return hideComponent(className);
                        });
                        break;
                    case 'previewContainer':
                        //that.$previewContainer.addClass('jqx-hidden');
                        hideComponent(controlledElements[5]);
                        break;
                    case 'alpha':
                        //controlledElements.slice(9, 11).forEach(item => that['$' + item].addClass('jqx-hidden'));
        ...
        */
        ...
        key: '_resizeHoneycombItems',
        value: function _resizeHoneycombItems() {
            var that = this;
        ...
        */
        ...
        key: '_getValue',
        value: function _getValue(type, randomColor) {
            var that = this,
                color = that._RGBA;
            var value = null,
                rgbaFragments = void 0;
            ...
            switch (type) {
                ...
                case 'HSVtoRGBAarray':
                    {
                        var alpha = that.displayMode === 'hexagonal' || that.displayMode === 'radial' || that.displayMode === 'palette' ? that._RGBA.a : 1;
                        ...
                        value = { r: parseInt(rgbaFragments[1]), g: parseInt(rgbaFragments[2]), b: parseInt(rgbaFragments[3]), a: that._toFixed(rgbaFragments[4]) };
                        break;
                    }
                case 'hsv':
        ...
        */
        ...
        key: '_instantUpdate',
        value: function _instantUpdate() {
            // rename?remove?update?
            var that = this,
                oldValue = that.value;
        ...
        */
        ...
        key: '_equalValues',
        value: function _equalValues(value1, value2) {
            var that = this;
        ...
        */
        ...
        key: '_toRGBA',
        value: function _toRGBA(value, ignoreAlpha) {
            // check if these transformations are required when there are central _HSV and _RGBA containers
            var that = this;
            var val = that.value;

            if (that._isRGBA(value)) {
                if (ignoreAlpha) {
                    var rgb = value.match(/(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})/gi);
            ...
            function hexToRgba(value) {
                var shortHexResult = /^#(.)(.)(.)$/gi.exec(value),
                ...
                return 'rgba(' + r + ', ' + g + ', ' + b + ', ' + (isNaN(a) ? '1' : (a / 255).toFixed(2)) + ')';
            }
        }
        ...
        */
        ...
        key: '_rgbaToRGB',
        value: function _rgbaToRGB(value) {
            var rgba = /rgba\((\d*.\d+|\d+),(\d*.\d+|\d+),(\d*.\d+|\d+)\,(\d*.\d+|\d+)\)/gi.exec(value.replace(/\s/g, ''));
            return 'rgb(' + rgba[1].trim() + ', ' + rgba[2].trim() + ', ' + rgba[3].trim() + ')';
        }
    }, {
        key: '_rgbaToHEX',
        value: function _rgbaToHEX(value) {
            var that = this,
        ...
        key: '_rgbArrayToHEX',
        value: function _rgbArrayToHEX(color) {
            var hex = '#';
            ...
            for (var i = 0; i < 3; i++) {
                var hexI = parseInt(color[i]).toString(16).toUpperCase();

                hexI = hexI.length === 1 ? 0 + hexI : hexI;
                hex = hex + hexI;
            }

            var alpha = parseFloat(color[3]);
        ...
        key: '_colorToHSV',
        value: function _colorToHSV(value) {
            var that = this;
            ...
            var rgbaFragments = /rgba\((\d*.\d+|\d+),(\d*.\d+|\d+),(\d*.\d+|\d+)\,(\d*.\d+|\d+)\)/gi.exec(value),
        ...
        key: '_HSVtoRGBA',
        value: function _HSVtoRGBA(hsv, a) {
            //  to return RGBA array or RGBA object!!!
            hsv = hsv || { h: 0, s: 0, v: 0 };
            var that = this,
        ...
        key: '_HSVRGBArefresh',
        value: function _HSVRGBArefresh(value) {
            var that = this;
        ...
        */
        ...
        key: '_toFixed',
        value: function _toFixed(value, symbols) {
        ...
        */
        ...
        key: '_validateInRange',
        value: function _validateInRange(value, min, max) {
        ...
        */
        ...
        key: '_getRGBAFromInputs',
        value: function _getRGBAFromInputs() {
            var that = this;
            var r = that.querySelector('input.r-channel'),
                g = that.querySelector('input.g-channel'),
                b = that.querySelector('input.b-channel'),
                a = that.querySelector('input.a-channel');
        ...
        key: '_setRGBAToInputs',
        value: function _setRGBAToInputs() {
            var that = this;
            Object.entries(that._RGBA).forEach(function (entry) {
                var input = that.querySelector('input.' + entry[0] + '-channel');
        ...
        */
        ...
        key: '_calcHueAngle',
        value: function _calcHueAngle(dx, dy) {
            return Math.atan2(dy, dx) * 180 / Math.PI;
        }
    }, {
        key: '_calcSatDistance',
        value: function _calcSatDistance(dx, dy) {
        ...
        get: function get() {
            return {
                ...
                'valueFormat': {
                    value: 'default',
                    allowedValues: ['rgb', 'rgba', 'hex', 'hsv', 'default'],
                    type: 'string'
                },
                'valueMember': {
                    value: 'value',
