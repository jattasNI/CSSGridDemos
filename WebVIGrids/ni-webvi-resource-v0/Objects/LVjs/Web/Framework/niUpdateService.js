...
//**********************************************************
import { JQXDateTimeValueConverter as DATETIME_VAL_CONVERTER } from './ValueConverters/niJQXDateTimeValueConverter.js';
import { INIArrayViewer } from '../Elements/ni-array-viewer-interface.js';
import { NIElement } from '../Elements/ni-element.js';
import { NI_SUPPORT } from './niSupport.js';
import { TaskTracker } from './niTaskTracker.js';
...
export class UpdateService {
    ...
    /**
        * Gets the the VirtualInstrumentModel with the given name.
        * @param {string} viName - The name of VI.
        * @returns {VirtualInstrumentModel} The VI model with the given name.
        * @throws Will throw if the VI is not found.
        */
    getVIModelByName(viName) {
        ...
        if (viModel !== undefined) {
            ...
            throw new Error(`No VI found with name: ${viName}. Existing names: ${Object.keys(viModels).join()}`);
        }
    }
    ...
    // Called by the WebAppModel
    reinitializeToDefault(_viModel, _controlModel) {
    }
    ...
    initialize(expectedState, tasksEnum) {
        ...
        const initTaskTracker = new TaskTracker(tasksEnum, function initComplete() {
    ...
    start(expectedState) {
        ...
        that.windowEngineCallbacks.getPropertyValue = function (argsArr) {
            ...
            if (nodes !== null && nodes.length !== 0) {
                ...
                if (INIArrayViewer.isNIArrayViewer(node.parentElement)) {
...
const INIT_TASKS_ENUM = UpdateService.InitTasksEnum;
const BROWSER_MESSAGE_ENUM = UpdateService.BrowserMessagesEnum;
