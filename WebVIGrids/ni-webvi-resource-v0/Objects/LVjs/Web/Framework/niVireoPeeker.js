...
export class PeekVisitor {
    ...
    visitAnalogWaveform(valueRef) {
        ...
        const timestampValue = this.visitTimestamp(valueRefObject.t0);
        const timeIntervalValue = this.visitDouble(valueRefObject.dt);
        const yValue = this.visitArray(valueRefObject.Y);
        const channelNameValueRef = this.vireo.eggShell.getVariantAttribute(valueRefObject.attributes, WaveformAttributeNameConstants.ChannelName);
        const channelName = channelNameValueRef === undefined ? undefined : this.vireo.eggShell.readString(channelNameValueRef);
        return { t0: timestampValue, dt: timeIntervalValue, Y: yValue, channelName: channelName };
    }
...
export const VireoPeeker = Object.freeze({
