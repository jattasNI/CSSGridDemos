 ...
 */
export class NIPackedClusterDataAdaptor {
    /**
    ...
    static PackDataValue(builder, dataToPack, niType) {
        ...
        for (let x = 0; x < fields.length; ++x) {
            if (!this.packInline(subTypes[x])) {
                const offset = NIPackedDataAdaptors.PackDataValue(builder, dataToPack[fields[x]], subTypes[x], true, x);
                offsets[x] = offset;
            }
        }
        PackedGenericClusterValue.startPackedClusterValue(builder, fields.length);
        for (let x = 0; x < fields.length; ++x) {
            if (this.packInline(subTypes[x])) {
                NIPackedDataAdaptors.PackDataValue(builder, dataToPack[fields[x]], subTypes[x], true, x);
            }
            else {
                builder.addFieldOffset(x, offsets[x], 0);
            }
        }
        return PackedGenericClusterValue.endPackedClusterValue(builder);
    }
    /**
    ...
    getUnpackedValue(clusterIndex, niType) {
        switch (niType.getName()) {
            ...
            case NITypeNames.ANALOGWAVEFORM:
                return NIPackedDataAdaptors.GetDataAdaptorFromIndex(this.packedData.getClusterElement(clusterIndex), niType, this.unpackingOptions).unpack();
