 ...
 */
import { NIPackedArrayUtils, NITypeUtils } from "./niPackedArrayUtils.js";
...
export class NIPackedArrayDataAdaptor {
    /**
...
export class NIArrayUnpacker {
    ...
    unpackObjectArray() {
        ...
        for (let x = 0; x < this.packedData.valueLength(); ++x) {
            ...
            const element = NIPackedDataAdaptors.GetDataAdaptorFromIndex(dataValue, subType, this.unpackingOptions);
    ...
    unpackBlittable1DArray() {
        ...
        const arrayValue = new NIFlatBuffers.PackedData.PackedBlittable1DArrayValue();
    ...
    unpackBlittableArray() {
        const arrayValue = new NIFlatBuffers.PackedData.PackedBlittableArrayValue();
        arrayValue.__init(this.packedData.bb_pos, this.packedData.bb);
    ...
    unpackBlittableSubArray(unpackingInfo, dimensionIndex) {
        if (dimensionIndex === unpackingInfo.dimensions.length - 1) {
            const startOffset = unpackingInfo.valueArray.byteOffset + unpackingInfo.consumedBytes;
            const subArray = NIPackedArrayUtils.createArrayFromByteArray(unpackingInfo.elementType, unpackingInfo.valueArray, startOffset, unpackingInfo.dimensions[unpackingInfo.dimensions.length - 1], this.unpackIntoJSArray());
...
export class NIArrayPacker {
    ...
    packObjectArray() {
        ...
        for (let x = 0; x < this.dataToPack.length; ++x) {
            ...
            const elementOffset = NIPackedDataAdaptors.PackDataValue(this.builder, currentValue, subType, false);
            offsets[x] = elementOffset;
        }
        const valuesOffset = NIFlatBuffers.PackedData.PackedArrayValue.createValueVector(this.builder, offsets);
        const dimensionOffset = NIFlatBuffers.PackedData.PackedArrayValue.createDimensionsVector(this.builder, this.dimensions);
        NIFlatBuffers.PackedData.PackedArrayValue.startPackedArrayValue(this.builder);
        NIFlatBuffers.PackedData.PackedArrayValue.addDimensions(this.builder, dimensionOffset);
        NIFlatBuffers.PackedData.PackedArrayValue.addValue(this.builder, valuesOffset);
        return NIFlatBuffers.PackedData.PackedArrayValue.endPackedArrayValue(this.builder);
    }
    /**
    ...
    packBlittableArray(byteArray) {
        const valueOffset = NIFlatBuffers.PackedData.PackedBlittable1DArrayValue.createValueVector(this.builder, byteArray);
        if (this.dimensions.length === 1) {
            return this.packBlittable1DArray(valueOffset);
        }
        const dimensionsOffset = NIFlatBuffers.PackedData.PackedBlittableArrayValue.createDimensionsVector(this.builder, this.dimensions);
    ...
    packBlittable1DArray(valueOffset) {
        NIFlatBuffers.PackedData.PackedBlittable1DArrayValue.startPackedBlittable1DArrayValue(this.builder);
        NIFlatBuffers.PackedData.PackedBlittable1DArrayValue.addValue(this.builder, valueOffset);
        NIFlatBuffers.PackedData.PackedBlittable1DArrayValue.addLength(this.builder, this.dataToPack.length);
        return NIFlatBuffers.PackedData.PackedBlittable1DArrayValue.endPackedBlittable1DArrayValue(this.builder);
    }
    /**
    ...
    packBlittableMultiDimensionalArray(valueOffset, dimensionsOffset) {
        NIFlatBuffers.PackedData.PackedBlittableArrayValue.startPackedBlittableArrayValue(this.builder);
        NIFlatBuffers.PackedData.PackedBlittableArrayValue.addValue(this.builder, valueOffset);
        NIFlatBuffers.PackedData.PackedBlittableArrayValue.addDimensions(this.builder, dimensionsOffset);
        return NIFlatBuffers.PackedData.PackedBlittableArrayValue.endPackedBlittableArrayValue(this.builder);
    }
}
/**
 * Manages state information about the current array unpacking information
 */
export class NIArrayUnpackingInfo {
    /**
     * Constructor
    ...
    constructor(elementSize, elementType, dimensions, valueArray) {
        ...
        this.dimensionByteCount = dimensions[dimensions.length - 1] * elementSize;
