...
export class VisualModel extends VisualComponentModel {
    constructor(id) {
        super(id);
    ...
    static get LABEL_RUNTIME_ID() {
        return "Label_RuntimeId";
    }
    static get KEY_FOCUS_G_PROPERTY_NAME() {
        return "KeyFocus";
    }
    get top() {
        return this._top;
    ...
    gPropertyNIType(gPropertyName) {
        switch (gPropertyName) {
            ...
            case VisualModel.VALUE_SIGNALING_G_PROPERTY_NAME:
                {
                    if (this.niType.getName() === NITypeNames.ENUM) {
                        // Since the enum type will be packed as underlying subtype, hence returning the subType here
                        return this.niType.getSubtype();
                    }
                    return this.niType;
                }
            ...
            case VisualModel.SIZE_G_PROPERTY_NAME:
                return new NIType({ name: NITypeNames.CLUSTER, fields: ['Width', 'Height'], subtype: [NITypeNames.SINGLE, NITypeNames.SINGLE] });
            case VisualModel.TOTAL_BOUNDS_G_PROPERTY_NAME:
                return new NIType({ name: NITypeNames.CLUSTER, fields: ['Left', 'Top', 'Width', 'Height'], subtype: [NITypeNames.SINGLE, NITypeNames.SINGLE, NITypeNames.SINGLE, NITypeNames.SINGLE] });
    ...
    reinitializeToDefault() {
        const viModel = this.getRoot();
        viModel.reinitializeToDefault(this);
    }
    /**
    ...
    isInFlexibleLayout() {
        ...
        else if (ownerModel instanceof VisualModel) {
            return ownerModel.isFlexibleLayout();
        }
        else if (ownerModel instanceof VisualComponentModel) {
            return false;
        }
    ...
    throwIfPropertyTypeIsUndefined(propertyName) {
        const propertyType = this.gPropertyNIType(propertyName);
        if (propertyType === undefined) {
            throw new Error('Property type not found for property: ' + propertyName + ' controlID: ' + this.niControlId);
        }
    }
}
