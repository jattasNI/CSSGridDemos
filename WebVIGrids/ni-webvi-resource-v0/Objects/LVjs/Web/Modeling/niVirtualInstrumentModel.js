...
import { VisualComponentModel } from './niVisualComponentModel.js';
...
export class VirtualInstrumentModel extends NIModel {
    ...
    addFrontPanelControlModel(element) {
        ...
        const modelKind = NIModelProvider.tagNameToModelKind(element.elementInfo.tagName);
        const controlModel = NIModelProvider.makeModel(modelKind, niControlId);
        controlModel.setOwner(parentModel);
        if (parentModel instanceof VisualModel) {
        ...
        const controlViewModel = NIModelProvider.makeViewModel(element, controlModel);
        if (element._temporaryModelSettingsHolder !== undefined) {
        ...
        this.updateModelAndViewModelMapping(niControlId, controlModel, controlViewModel);
    ...
    /**
        * Add mapping between controlModel and controlViewModel using niControlId.
        * @param niControlId controlId of the control.
        * @param controlModel controlModel to which the controldId is associated with.
        * @param controlViewModel controlViewModel to which the controldId is associated with.
    */
    updateModelAndViewModelMapping(niControlId, controlModel, controlViewModel) {
    ...
    removeFrontPanelControl(element) {
        ...
        if (controlModel instanceof VisualComponentModel === false ||
            controlViewModel instanceof VisualComponentViewModel === false) {
        ...
        if (controlModel.getOwner() instanceof VisualModel) {
    ...
    reinitializeToDefault(controlModel) {
        const webAppModel = this.getOwningWebApplication();
        webAppModel.reinitializeToDefault(this, controlModel);
    }
    getNameVireoEncoded() {
        return vireoHelpers.staticHelpers.encodeIdentifier(this.viName);
    }
    enableEvents() {
        const webAppModel = this.getOwningWebApplication();
