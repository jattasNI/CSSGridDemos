...
//****************************************
import { EditorRuntimeUpdateService } from '../Framework/niEditorRuntimeUpdateService.js';
import { EditorUpdateService } from '../Framework/niEditorUpdateService.js';
...
const SERVICE_STATE_ENUM = WebApplicationStates.ServiceStateEnum;
...
export class WebApplicationModel extends NIModel {
    constructor(id) {
        super(id);
    ...
    initializeService() {
        ...
        updateServiceTable[PANEL_LOCATION_ENUM.IDE_EDIT][PANEL_ENGINE_ENUM.NATIVE] = EditorUpdateService;
        updateServiceTable[PANEL_LOCATION_ENUM.IDE_RUN][PANEL_ENGINE_ENUM.NATIVE] = EditorRuntimeUpdateService;
        updateServiceTable[PANEL_LOCATION_ENUM.BROWSER][PANEL_ENGINE_ENUM.NATIVE] = RemoteUpdateService;
        updateServiceTable[PANEL_LOCATION_ENUM.IDE_EDIT][PANEL_ENGINE_ENUM.VIREO] = EditorUpdateService;
        updateServiceTable[PANEL_LOCATION_ENUM.IDE_RUN][PANEL_ENGINE_ENUM.VIREO] = LocalUpdateService;
        updateServiceTable[PANEL_LOCATION_ENUM.BROWSER][PANEL_ENGINE_ENUM.VIREO] = LocalUpdateService;
        ...
        if (updateServiceTable[that.location] !== undefined && updateServiceTable[that.location][that.engine] !== undefined) {
            ...
            that.updateService = new TestUpdateService(config);
        }
        else {
        ...
        setTimeout(function () {
            that.updateService.initialize(that._vireoWasmPath);
        }, 0);
    }
    start() {
    ...
    reinitializeToDefault(viModel, controlModel) {
        if (this.updateService !== undefined) {
            this.updateService.reinitializeToDefault(viModel, controlModel);
        }
    }
}
