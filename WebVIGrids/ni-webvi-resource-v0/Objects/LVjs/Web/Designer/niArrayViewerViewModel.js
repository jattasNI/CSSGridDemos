...
//****************************************
import { ArrayViewer } from '../Elements/ni-array-viewer.js';
    ...
    for (newProp in srcRenderBuffer.properties) {
        if (srcRenderBuffer.properties.hasOwnProperty(newProp)) {
            if (srcElement.valuePropertyDescriptor !== undefined &&
                ...
                srcElement.parentElement instanceof ArrayViewer) {
                continue;
            }
...
const updateElementsFromRenderBuffer = function (elements, renderBuffer, srcElement, isArrayElement) {
    ...
    for (i = 0; i < elements.length; i++) {
        curBuffer = RenderEngine.getOrAddRenderBuffer(elements[i]);
        copyRenderBuffer(srcElement, renderBuffer, curBuffer, isArrayElement);
        RenderEngine.enqueueDomUpdate(elements[i]);
    }
};
...
const findRootArrayViewModel = function (controlModel, viRef) {
    ...
    while (!currControlModel.isDataItemBoundControl()) {
        if (currControlModel instanceof ArrayViewerModel) {
    ...
    const viModel = viReferenceService.getVIModelByVIRef(viRef);
...
const createElementShims = function (rootArrayViewModel, childViewModel) {
    ...
    const isArrayElement = owner instanceof ArrayViewerModel;
    return {
        setCallback: function () {
            childViewModel.userInteractionChanged = function (newState, operationKind) {
                ...
                if (updateClonesAfterInteractionEnded) {
                    const renderBuffer = RenderEngine.getOrAddRenderBuffer(childViewModel.element);
            ...
            childViewModel.modelPropertyChanged = function (propertyName) {
                ...
                if (propertyName === 'left' || propertyName === 'top' || propertyName === 'width' || propertyName === 'height') {
                    ...
                    if (childViewModel.element.valuePropertyDescriptor === undefined ||
                        ...
                        (childViewModel.element.parentElement instanceof ArrayViewer) === false) {
                        elements = viewModelData.getClones();
            ...
            childViewModel.onChildViewModelAdded = function (viewModel) {
                ...
                const viViewModel = viReferenceService.getVIViewModelByVIRef(viewModel.element.viRef);
            ...
            childViewModel.onChildViewModelRemoved = function (viewModel) {
                ...
                if (!(childViewModel.model instanceof ArrayViewerModel)) {
...
export class ArrayViewerViewModel extends VisualViewModel {
    ...
    bindToView() {
        ...
        that.element.addEventListener('array-size-changed', function (evt) {
            const ideEditPanelLocation = WebApplicationStates.PanelLocationEnum.IDE_EDIT;
            if (viReferenceService.getWebAppModelByVIRef(this.viRef).location !== ideEditPanelLocation) {
                if (evt.target === that.element) {
        ...
        if (that.model.childModels.length > 0) {
            ...
            viModel = viReferenceService.getVIModelByVIRef(that.element.viRef);
    ...
    initializeElementViewModel(childViewModel) {
        ...
        if (childViewModel instanceof VisualViewModel) {
            ...
            for (i = 0; i < childModels.length; i++) {
                ...
                viModel = viReferenceService.getVIModelByVIRef(this.element.viRef);
...
NIModelProvider.registerViewModel(ArrayViewerViewModel, ArrayViewer, ArrayViewerModel);
