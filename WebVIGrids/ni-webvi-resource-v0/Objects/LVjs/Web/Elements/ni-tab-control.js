...
const applySelectedIndex = function (target) {
    ...
    if (hasValidTabs && selectedIndexInValidTabsRange && hasValidHeaderContainer) {
        ...
        for (i = 0; i < target.children.length; i++) {
            if (INITabItem.isNITabItem(target.children[i])) {
...
// None
export class TabControl extends LayoutControl {
    ...
    addAllProperties(targetPrototype) {
        ...
        proto.addProperty(targetPrototype, {
            propertyName: 'tabSelectorHidden',
            defaultValue: false
        });
    ...
    updateDisableStateForChildren() {
        for (let i = 0; i < this.children.length; i++) {
            ...
            if (INITabItem.isNITabItem(tabItem)) {
...
NIElementRegistrationService.registerElement(TabControl, NIElementRegistrationWeightEnum.CONTAINER);
VisualComponent.defineElementInfo(TabControl.prototype, 'ni-tab-control', 'HTMLNITabControl');
