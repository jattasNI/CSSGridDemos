...
import { NumericValueConverter as NUM_VAL_CONVERTER } from '../Framework/ValueConverters/niNumericValueConverter.js';
...
const resetAllDisabledItems = function (dropdownElement) {
    const childItems = dropdownElement.items;
    if (childItems === undefined) {
        return;
    }
    for (let i = 0; i < childItems.length; i++) {
        if (childItems[i].disabled) {
            childItems[i].disabled = false;
        }
    }
};
export class NumericValueSelector extends Visual {
    ...
    setDisabledIndexes(dropdownElement) {
        resetAllDisabledItems(dropdownElement);
        const childItems = dropdownElement.items;
        const disabledIndexes = this.disabledIndexesArray;
        for (let i = 0; i < disabledIndexes.length; i++) {
            if (disabledIndexes[i] < childItems.length &&
                disabledIndexes[i] >= 0 &&
                !childItems[disabledIndexes[i]].disabled) {
                childItems[disabledIndexes[i]].disabled = true;
            }
        }
    }
