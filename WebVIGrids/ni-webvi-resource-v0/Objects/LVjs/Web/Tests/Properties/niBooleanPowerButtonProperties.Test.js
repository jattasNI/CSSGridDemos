//****************************************
// G Property Tests for BooleanPowerButtonModel class
...
describe('A BooleanPowerButton control', function () {
    ...
    beforeEach(function (done) {
        Object.freeze(booleanPowerButtonSettings);
    ...
    it('property read for Value returns the current value.', function () {
        ...
        expect(currentValue).toEqual(booleanPowerButtonSettings.value);
    });
    ...
    it('property set for valueSignaling calls controlChanged function of updateService with correct arguments', function (done) {
        ...
        testHelpers.runAsync(done, function () {
            ...
            expect(updateService.controlChanged).toHaveBeenCalledWith(viModel, controlModel, 'value', newValue, booleanPowerButtonSettings.value);
        });
    });
    it('property set for valueSignaling with the same value as the current value calls controlChanged function of updateService', function (done) {
        const newValue = booleanPowerButtonSettings.value;
        testHelpers.runAsync(done, function () {
            spyOn(updateService, 'controlChanged');
            viewModel.setGPropertyValue(VisualModel.VALUE_SIGNALING_G_PROPERTY_NAME, newValue);
    ...
    it('property set for text with empty array will leave the control element unchanged.', function (done) {
        ...
        testHelpers.runMultipleAsync(done, function () {
            viewModel.setGPropertyValue(BooleanControlModel.TEXT_G_PROPERTY_NAME, newText);
            expect(controlModel.trueContent).toEqual(booleanPowerButtonSettings.trueContent);
            expect(controlModel.falseContent).toEqual(booleanPowerButtonSettings.falseContent);
        }, function () {
            expect(controlElement.trueContent).toEqual(booleanPowerButtonSettings.trueContent);
            expect(controlElement.falseContent).toEqual(booleanPowerButtonSettings.falseContent);
        });
    });
    it('property read for Position returns the current position.', function () {
        ...
        const expectedPosition = {
            "Left": parseInt(booleanPowerButtonSettings.left),
            "Top": parseInt(booleanPowerButtonSettings.top)
        };
        expect(position).toEqual(expectedPosition);
    });
    it('property set for Position updates model.', function (done) {
        const newPosition = { Left: 103, Top: 203 };
        testHelpers.runAsync(done, function () {
    ...
    it('property set for Position updates control element.', function (done) {
        const newPosition = { Left: 152, Top: 252 };
        testHelpers.runMultipleAsync(done, function () {
            viewModel.setGPropertyValue(VisualModel.POSITION_G_PROPERTY_NAME, newPosition);
        }, function () {
    ...
    it('property get for true color throws an exception.', function () {
        const errorMessage = NI_SUPPORT.i18n('msg_PROPERTY_CANNOT_BE_GET', BooleanControlModel.TRUE_COLOR_G_PROPERTY_NAME);
        expect(function () {
            viewModel.getGPropertyValue(BooleanControlModel.TRUE_COLOR_G_PROPERTY_NAME);
        }).toThrowError(Error, errorMessage);
    });
    it('property get for false color throws an exception.', function () {
        const errorMessage = NI_SUPPORT.i18n('msg_PROPERTY_CANNOT_BE_GET', BooleanControlModel.FALSE_COLOR_G_PROPERTY_NAME);
        expect(function () {
            viewModel.getGPropertyValue(BooleanControlModel.FALSE_COLOR_G_PROPERTY_NAME);
        }).toThrowError(Error, errorMessage);
    });
});
//# sourceMappingURL=niBooleanPowerButtonProperties.Test.js.map